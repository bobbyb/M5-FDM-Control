...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by AnkerMake Studio ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char ankerstudio_gcode_header[] = "; generated by AnkerMake Studio ";
        std::string header;
        ...
        while (std::getline(ifs, header)) {
            if (strncmp(slic3r_gcode_header, header.c_str(), strlen(slic3r_gcode_header)) == 0) {
                header_found = true;
                break;
            } else if (strncmp(ankerstudio_gcode_header, header.c_str(), strlen(ankerstudio_gcode_header)) == 0) {
                // Parse PrusaSlicer version.
                size_t i = strlen(ankerstudio_gcode_header);
    ...
    if (has_delimiters)
    {
        ...
        // ; ankerstudio_config = begin
        // ...
        // ; ankerstudio_config = end
        ...
        if (! begin_found) 
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; ankerstudio_config = begin\" not found when reading %1%", file));
    }
    else
    {
...
size_t DynamicConfig::remove_nil_options()
{
	...
	for (auto it = options.begin(); it != options.end();)
		if (it->second->is_nil()) {
            opttopo_del(it->second.get()); //  add  @2023-06-26 by ChunLian
			it = options.erase(it);
			++ cnt_removed;
		} else
			++ it;
...
//        throw ConfigurationError(std::string("Invalid option name: ") + opt_key);
    ...
    opttopo_add(this->options[opt_key].get(), opt_key); //  add @2023-06-26 by ChunLian
    return opt;
}
...
bool DynamicConfig::set_key_value(const std::string& opt_key, ConfigOption* opt)
{
    ...
    if (it == this->options.end()) {    //  add new value
        this->options[opt_key].reset(opt);  
        opttopo_add(this->options[opt_key].get(), opt_key); //  add  @2023-06-26 by ChunLian
        return true;
    }
    else {                              //  change value  add @2023-06-12 by ChunLian
        //it->second.reset(opt);
        it->second->set(opt);     
        delete opt;
        return false;
...
void DynamicConfig::opttopo_add(const ConfigOption * const opt, t_config_option_key key) {
    opttopo[opt] = key; 
}
void DynamicConfig::opttopo_del(const ConfigOption * const opt) { opttopo.erase(opt); }
void DynamicConfig::opttopo_clear() { opttopo.clear(); }
void DynamicConfig::opttopo_reset() { opttopo_clear();  for (const auto& it : options) opttopo_add(it.second.get(), it.first); }
void DynamicConfig::opttopo_filter(const ConfigOption* const opt) {
    if (opttopo_change) {
        opttopo_change(opttopo[opt], opt);
    }
    else if (true) {
        ANKER_LOG_DEBUG << "[TEST:Config Value Change] " << opttopo[opt] << " ==== " << opt->serialize();
    }
}
