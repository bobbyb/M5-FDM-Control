...
// 0 : 3MF files saved by older AnkerStudio or the painting gizmo wasn't used. No version definition in them.
...
const std::string LAYER_CONFIG_RANGES_FILE = "Metadata/Anker_Studio_layer_config_ranges.xml";
...
static constexpr const char* AK_ASSEMBLE_TAG = "assemble";
static constexpr const char* AK_ASSEMBLE_ITEM_TAG = "assemble_item";
static constexpr const char* AK_SLICE_HEADER_TAG = "header";
static constexpr const char* AK_SLICE_HEADER_ITEM_TAG = "header_item";
...
Slic3r::Vec3d get_offset_from_3mf_specs_string(const std::string& vec_str)
{
...
namespace Slic3r {
    ...
    class _3MF_Importer : public _3MF_Base
    {
        ...
        // Semantic version of AnkerStudio, that generated this 3MF.
        boost::optional<Semver> m_ankerstudio_generator_version;
    ...
    public:
        ...
        boost::optional<Semver> ankerstudio_generator_version() const { return m_ankerstudio_generator_version; }
    ...
    bool _3MF_Importer::_load_model_from_file(const std::string& filename, Model& model, DynamicPrintConfig& config, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        if (m_version == 0) {
            // if the 3mf was not produced by AnkerStudio and there is more than one instance,
...
//        model.adjust_min_z();
    ...
    void _3MF_Importer::_handle_start_config_xml_element(const char* name, const char** attributes)
    {
        ...
        else if (::strcmp(AK_ASSEMBLE_TAG, name) == 0)
            res = _handle_start_assemble(attributes, num_attributes);
        else if (::strcmp(AK_ASSEMBLE_ITEM_TAG, name) == 0)
    ...
    bool _3MF_Importer::_handle_end_model()
    {
        ...
        if (m_version == 0) {
            // if the 3mf was not produced by AnkerStudio and there is only one object,
    ...
    bool _3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        m_curr_object.geometry.mmu_segmentation.push_back(get_attribute_value_string(attributes, num_attributes, MM_SEGMENTATION_ATTR));
        return true;
    }
    ...
    bool _3MF_Importer::_handle_start_item(const char** attributes, unsigned int num_attributes)
    {
        ...
        return !m_load_model || _create_object_instance(object_id, transform, printable, 1);
    }
    ...
    bool _3MF_Importer::_handle_end_metadata()
    {
        if (m_curr_metadata_name == SLIC3RPE_3MF_VERSION) {
            ...
            if (boost::starts_with(m_curr_characters, "AnkerStudio-"))
                m_ankerstudio_generator_version = Semver::parse(m_curr_characters.substr(12));
        } else if (m_curr_metadata_name == SLIC3RPE_FDM_SUPPORTS_PAINTING_VERSION) {
            m_fdm_supports_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                _u8L("The selected 3MF contains FDM supports painted object using a newer version of M5 FDM Control and is not compatible."));
        } else if (m_curr_metadata_name == SLIC3RPE_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                _u8L("The selected 3MF contains seam painted object using a newer version of M5 FDM Control and is not compatible."));
        } else if (m_curr_metadata_name == SLIC3RPE_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                _u8L("The selected 3MF contains multi-material painted object using a newer version of M5 FDM Control and is not compatible."));
        }
    ...
    bool _3MF_Importer::_handle_start_assemble_item(const char** attributes, unsigned int num_attributes)
    {
        ...
        int object_id = get_attribute_value_int(attributes, num_attributes, OBJECT_ID_ATTR);
        int instance_id = get_attribute_value_int(attributes, num_attributes, INSTANCEID_ATTR);
        ...
        Transform3d transform = get_transform_from_3mf_specs_string(get_attribute_value_string(attributes, num_attributes, TRANSFORM_ATTR));
        Vec3d ofs2ass = get_offset_from_3mf_specs_string(get_attribute_value_string(attributes, num_attributes, OFFSET_ATTR));
    ...
    bool _3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            if (m_ankerstudio_generator_version && 
                *m_ankerstudio_generator_version >= *Semver::parse("2.4.0-alpha1") &&
                *m_ankerstudio_generator_version < *Semver::parse("2.4.0-alpha3"))
            ...
            if (m_version == 0) {
                // if the 3mf was not produced by AnkerStudio and there is only one instance,
    ...
    bool _3MF_Exporter::_save_model_to_file(const std::string& filename, Model& model, const DynamicPrintConfig* config, const ThumbnailData* thumbnail_data, const SliceModelData* slice_data)
    {
        ...
        // The content of this file is the same for each AnkerStudio 3mf.
        ...
        // Adds custom gcode per height file ("Metadata/Anker_Studio_custom_gcode_per_print_z.xml").
    ...
    bool _3MF_Exporter::_add_mesh_to_object_stream(mz_zip_writer_staged_context &context, ModelObject& object, VolumeToOffsetsMap& volumes_offsets)
    {
        ...
        for (ModelVolume* volume : object.volumes) {
            ...
            for (int i = 0; i < int(its.indices.size()); ++ i) {
                ...
                std::string mm_painting_data_string = volume->mmu_segmentation_facets.get_triangle_as_string(i);
                ...
                if (obj != nullptr) {
                    for (int instance_idx = 0; instance_idx < obj->instances.size(); ++instance_idx) {
                        if (obj->instances[instance_idx]->is_assemble_initialized()) {
                            stream << "   <" << AK_ASSEMBLE_ITEM_TAG << " " << OBJECT_ID_ATTR << "=\"" << obj_metadata.first << "\" ";
...
static void handle_legacy_project_loaded(unsigned int version_project_file, DynamicPrintConfig& config, const boost::optional<Semver>& ankerstudio_generator_version)
{
    ...
    if (ankerstudio_generator_version && *ankerstudio_generator_version >= *Semver::parse("2.5.0-alpha2") && *ankerstudio_generator_version <= *Semver::parse("2.5.0-alpha3")) {
...
bool load_3mf(const char* path, DynamicPrintConfig& config, ConfigSubstitutionContext& config_substitutions, Model* model, bool check_version)
{
    ...
    importer.log_errors();
    handle_legacy_project_loaded(importer.version(), config, importer.ankerstudio_generator_version());
