...
private:
    ...
    // Blocking POST to <baseUrl><path> with X-Auth-Token + Gtoken(md5(userId)) +
    // client-id headers. Returns true only on transport success + HTTP 200.
...
private:
    ...
    // App resources directory; holds crt/make-us.crt, the broker's pinned cert.
    ...
    // Message Center (notification bell): the UI registers these; GetMsgCenterStatus/
    // GetMsgCenterRecords fetch /v1/slicer/news/* and invoke them from a worker thread.
    CallBack_MsgCenterStatus m_msgStatusCb;
    CallBack_MsgCenterRecords m_msgRecordsCb;
    ...
    // Fired once the device list is ready so the slicer window's chooser can drop its
    // loading mask; without it the chooser spins until its own timeout expires.
    sendShowDeviceListDialog_T m_showDeviceListCb;

    // Fired when a send starts, so the UI leaves the slicer tab and shows the printer
    // the job is going to.
    sendSigToSwitchPrintPage_T m_switchPrintPageCb;

    // Fired when an API request fails at the transport level, so a dead network or a
    // rejected token surfaces instead of the app silently doing nothing.
    sendSigHttpError_T m_httpErrorCb;
