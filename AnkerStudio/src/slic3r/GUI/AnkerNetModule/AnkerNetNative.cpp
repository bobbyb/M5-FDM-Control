...
namespace {
// Lowercase-hex MD5 of a string. The Anker API gateway requires a "Gtoken" header on
// every request equal to md5(user_id) -- /v1/slicer/* endpoints reject requests without
// it ("banned"); /v1/app/* tolerate its absence. Empty input yields md5("").
std::string md5Hex(const std::string& in)
{
    ...
    for (unsigned int i = 0; i < len; ++i) {
        ...
        out.push_back(hexd[digest[i] & 0x0f]);
    }
    return out;
}
...
// Unix epoch seconds -> local "YYYY/M/D HH:MM:SS", matching the format the Message
// Center's time label is laid out for. The API hands back a bare epoch; handing that
// straight to the label just prints a 10-digit number.
std::string formatEpochLocal(long long epochSeconds)
{
    if (epochSeconds <= 0)
        return std::string();
...
bool AnkerNetNativeImpl::Init(AnkerNetInitPara para)
{
    ...
    m_resourcesDir = para.resourcesDir;
    m_init = true;
...
void AnkerNetNativeImpl::SetsendSigHttpError(sendSigHttpError_T function)
{
    ...
    m_httpErrorCb = function;
}

// Message Center badge counts: POST /v1/slicer/news/get_news_num ->
// data:{news_num_official, news_num_printer, ...}. Runs on a worker thread; the callback
// (registered by the UI) posts a wx event, so calling it off-thread is safe.
void AnkerNetNativeImpl::GetMsgCenterStatus()
{
    std::string authToken, userId, baseUrl;
    CallBack_MsgCenterStatus cb;
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (!m_login.logined)
            return;
    ...
    std::thread([this, authToken, userId, baseUrl, cb]() {
        std::string resp;
        if (!httpPostV1(baseUrl, "/v1/slicer/news/get_news_num", authToken, userId, "", resp))
            return;
        try {
            pt::ptree root;
            ...
            if (root.get<int>("code", -1) != 0)
                return;
            int official = root.get<int>("data.news_num_official", 0);
            int printer = root.get<int>("data.news_num_printer", 0);
            cb(official, printer);
        } catch (const pt::json_parser_error&) {
            std::fprintf(stderr, "[AnkerNetNative] get_news_num: bad JSON\n");
        }
    }).detach();
}

// Message Center list: POST /v1/slicer/news/get_news {news_type,num,page} ->
// data:[{news_id,title,content,url,error_code,create_time,is_news,...}].
void AnkerNetNativeImpl::GetMsgCenterRecords(const int& newType, const int& num, const int& page, bool /*isSyn*/)
{
    std::string authToken, userId, baseUrl;
    CallBack_MsgCenterRecords cb;
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (!m_login.logined)
            return;
    ...
    std::thread([this, authToken, userId, baseUrl, newType, num, page, cb]() {
        std::ostringstream body;
        body << "{\"news_type\":" << newType << ",\"num\":" << num << ",\"page\":" << page << "}";
        std::string resp;
        if (!httpPostV1(baseUrl, "/v1/slicer/news/get_news", authToken, userId, body.str(), resp))
            return;

        std::vector<MsgCenterItem> items;
        try {
            pt::ptree root;
            std::istringstream iss(resp);
            pt::read_json(iss, root);
            ...
            if (auto data = root.get_child_optional("data")) {
                for (const auto& kv : *data) {
                    const pt::ptree& n = kv.second;
                    MsgCenterItem it;
                    it.msgID = n.get<int>("news_id", 0);
                    it.msgTitle = n.get<std::string>("title", "");
                    it.msgContent = n.get<std::string>("content", "");
                    it.msgUrl = n.get<std::string>("url", "");
                    it.msgCreateTime = formatEpochLocal(n.get<long long>("create_time", 0));
                    it.msgErrorCode = n.get<std::string>("error_code", "");
                    it.alarm_type = n.get<int>("news_type", 0);
                    // is_news: 1 = unread announcement/alert, 0 = already surfaced.
                    it.msgIsNew = n.get<int>("is_news", 0) != 0;
                    items.push_back(it);
                }
            }
        } catch (const pt::json_parser_error&) {
            std::fprintf(stderr, "[AnkerNetNative] get_news: bad JSON\n");
        }
        cb(items);
    }).detach();
}
...
// Delete Message Center entries server-side: POST /v1/slicer/news/delete_user_news
// {news_ids:[...]}. Endpoint and field name recovered from the stock plugin's symbol
// table. Without this the UI only hid rows locally and they returned on the next fetch.
//
// isSyn is honoured rather than ignored: the dialog passes true when it has deleted
// every visible row and immediately re-fetches the list, so returning before the
// server has applied the delete would repopulate it with the rows just removed.
void AnkerNetNativeImpl::removeMsgByIds(const std::vector<int>& msgList, bool isSyn)
{
    ...
    auto doDelete = [this, authToken, userId, baseUrl, payload = body.str()]() {
        std::string resp;
        if (!httpPostV1(baseUrl, "/v1/slicer/news/delete_user_news", authToken, userId, payload, resp)) {
            std::fprintf(stderr, "[AnkerNetNative] delete_user_news: request failed\n");
            return;
        }
        try {
            pt::ptree root;
            std::istringstream iss(resp);
            pt::read_json(iss, root);
            const int code = root.get<int>("code", -1);
            if (code != 0)
                std::fprintf(stderr, "[AnkerNetNative] delete_user_news: code %d\n", code);
        } catch (const pt::json_parser_error&) {
            std::fprintf(stderr, "[AnkerNetNative] delete_user_news: bad JSON\n");
        }
    };
...
void AnkerNetNativeImpl::AsyRefreshDeviceList()
{
    ...
    std::thread([this, authToken, userId, baseUrl]() {
        std::list<DeviceObjectBasePtr> devices = fetchDeviceList(authToken, userId, baseUrl);
...
// Called by Plater::a_key_print_clicked() immediately before it opens the device
// chooser modally. The chooser comes up behind a loading mask that is cleared only by
// the show-device-list callback, so as a no-op this left the slicer window's Print
// button sitting on a spinner until the mask timed out. Refresh the list, then fire
// the callback -- the refresh takes a network round-trip, by which time the modal
// chooser is up and able to receive the event.
void AnkerNetNativeImpl::AsyOneKeyPrint()
{
    std::thread([this]() {
        ...
        sendShowDeviceListDialog_T showCb;
        {
            std::lock_guard<std::mutex> lock(m_deviceMutex);
            if (!devices.empty())
                m_devices = std::move(devices);
            showCb = m_showDeviceListCb;
        }
...
void AnkerNetNativeImpl::StartP2pOperator(P2POperationType type, const std::string& sn, const std::string& filePath)
{
    ...
    // Send-to-printer. Two callers reach this with the same intent:
    //   P2P_TRANSFER_LOCAL_FILE   -- Device tab, passes the gcode path directly.
    //   P2P_TRANSFER_PREVIEW_FILE -- slicer/preview window's Print button, which
    //                                passes an EMPTY path and instead stashes the
    //                                exported gcode via SetGcodePath() beforehand
    //                                (Plater::ExportGacode).
    // Handling only the former meant printing from the slicer window returned here
    // silently and nothing was ever sent.
    if (type != P2P_TRANSFER_LOCAL_FILE && type != P2P_TRANSFER_PREVIEW_FILE)
        return;
    ...
    if (srcPath.empty()) {
        std::fprintf(stderr, "[AnkerNetNative] no gcode path for transfer\n");
        return;
    }
    ...
    // Leave the slicer tab for the Device tab as soon as the send is committed, so the
    // user watches the transfer and print rather than the plate they just sliced. Posted
    // before the worker starts: the UI handler only posts a wx event, and the chooser's
    // EndModal() runs right after this returns, so the switch lands once the modal is
    // gone. Without this the app sat on the slicer tab for the whole upload.
    {
        sendSigToSwitchPrintPage_T switchCb;
        {
            std::lock_guard<std::mutex> lock(m_deviceMutex);
            switchCb = m_switchPrintPageCb;
        }
        if (switchCb)
            switchCb(sn);
    }
    ...
    std::thread([this, sn, ip, duid, name, userId, machineId, srcPath, dsk, p2pConn]() {
        ...
        std::string fileName = boost::filesystem::path(srcPath).filename().string();
...
bool AnkerNetNativeImpl::httpPostV1(const std::string& baseUrl, const std::string& path,
    const std::string& authToken, const std::string& userId, const std::string& jsonBody,
    std::string& outResponse) const
{
    ...
    // Gateway signature the production app stamps on every request: md5(user_id).
    const std::string gtokenHeader = "Gtoken: " + md5Hex(userId);

    struct curl_slist* headers = nullptr;
    headers = curl_slist_append(headers, "Content-Type: application/json");
    headers = curl_slist_append(headers, authHeader.c_str());
    headers = curl_slist_append(headers, gtokenHeader.c_str());
    ...
    if (res != CURLE_OK || httpCode != 200) {
        ...
        // Tell the UI why. Without this every transport failure was silent: a dead
        // network or a rejected token just left the device list empty with no
        // explanation and no prompt to sign in again.
        //
        // Only transport-level facts are classified here. 401/403 is an unambiguous
        // rejection of our token and maps to UserNotIdentified, which drives the UI's
        // re-login path. Application-level failures (HTTP 200 with a non-zero JSON
        // "code") are deliberately NOT reported as auth errors -- the codes Anker uses
        // for an expired session have not been observed, and guessing risks logging
        // the user out on an unrelated error.
        HttpError err = UnknownError;
        if (res == CURLE_COULDNT_RESOLVE_HOST || res == CURLE_COULDNT_RESOLVE_PROXY)
            err = ResolveHostError;
        else if (httpCode == 401 || httpCode == 403)
            err = UserNotIdentified;

        sendSigHttpError_T cb;
        {
            std::lock_guard<std::mutex> lock(m_deviceMutex);
            cb = m_httpErrorCb;
        }
...
std::list<DeviceObjectBasePtr> AnkerNetNativeImpl::fetchDeviceList(
    const std::string& authToken, const std::string& userId, const std::string& baseUrl) const
{
    std::list<DeviceObjectBasePtr> result;

    std::string listResp;
    if (!httpPostV1(baseUrl, "/v1/app/query_fdm_list", authToken, userId, "{}", listResp))
        return result;
    ...
    std::string dskResp;
    if (httpPostV1(baseUrl, "/v1/app/equipment/get_dsk_keys", authToken, userId, body, dskResp)) {
        try {
            pt::ptree droot;
...
void AnkerNetNativeImpl::startMonitoring()
{
    ...
    {
        // Pin the broker's own self-signed certificate, shipped in the app resources.
        std::string resourcesDir;
        {
            std::lock_guard<std::mutex> lock(m_mutex);
            resourcesDir = m_resourcesDir;
        }
        if (!resourcesDir.empty()) {
            const auto ca = boost::filesystem::path(resourcesDir) / "crt" / "make-us.crt";
            if (boost::filesystem::exists(ca))
                client->setCaFile(ca.string());
            else
                std::fprintf(stderr, "[AnkerNetNative] broker cert missing at %s\n",
                    ca.string().c_str());
        }
    }
