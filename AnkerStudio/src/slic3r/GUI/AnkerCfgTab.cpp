...
/*
#ifndef __APPLE__
wxFont ankerFont(9, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTSTYLE_NORMAL, false, wxT("Microsoft YaHei"));
#else
wxFont ankerFont(11, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTSTYLE_NORMAL, false, wxT("Square"));
#endif // !__APPLE__
*/
	...
	#include <CommCtrl.h>
#endif // WIN32
wxDEFINE_EVENT(wxCUSTOMEVT_ANKER_SAVE_PRESET, wxCommandEvent);
namespace Slic3r {
namespace GUI {

#define DEFAULT_PRINT_PRESET_KEYWORDS "Normal"
    ...
    m_parent(parent), m_type(type), m_title(title)
{
    ...
    wxGetApp().ankerTabsList.push_back(this);
...
// sub new
void AnkerTab::create_preset_tab()
{
    this->SetBackgroundColour(wxColour(51, 52, 56));
#ifdef __WINDOWS__
    ...
    // Vertical sizer to hold the choice menu and the r#include "AnkerCfgTab.hpp"est of the page.
#ifdef __WXOSX__
   ...
   // sizer->SetSizeHints(panel);
    ...
    add_scaled_button(panel, &m_btn_save_preset, "save_setting");
    // add by allen for ankerCfgDlg for use right menu to rename or delete preset both
    add_scaled_button(panel, &m_btn_rename_preset, "edit");
    add_scaled_button(panel, &m_btn_delete_preset, "cross");
    if (m_type == Preset::Type::TYPE_PRINTER)
    {
        add_scaled_button(panel, &m_btn_edit_ph_printer, "cog");
        m_btn_edit_ph_printer->Hide();
    }

    m_show_incompatible_presets = false;

    add_scaled_button(panel, &m_btn_hide_incompatible_presets, "flag_green");
    m_btn_hide_incompatible_presets->Hide();
    ...
    add_scaled_bitmap(this, m_bmp_empty, "empty");

    fill_icon_descriptions();
    set_tooltips_text();

    add_scaled_button(panel, &m_undo_btn, m_bmp_empty.name());
    ...
    m_h_buttons_sizer->Add(m_undo_btn, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT);
    m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    m_h_buttons_sizer->Add(m_btn_rename_preset, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | int(4 * scale_factor));
    m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    m_h_buttons_sizer->Add(m_btn_delete_preset, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | int(4 * scale_factor));
    m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    ...
    m_h_buttons_sizer->Add(m_btn_hide_incompatible_presets, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT| int(8 * scale_factor));
    //m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    m_h_buttons_sizer->Add(m_question_btn, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT| int(8 * scale_factor));
    //m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    m_h_buttons_sizer->Add(m_undo_to_sys_btn, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT| int(8 * scale_factor));
    // m_h_buttons_sizer->AddSpacer(int(8 * scale_factor));
    m_h_buttons_sizer->Add(m_btn_compare_preset, 0, wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT| int(8 * scale_factor));
    ...
    if (m_mode_sizer) {
        ...
        // add by allen for hide mode button in mac
...
//        m_hsizer->Add(mode_sizer, 1, wxALIGN_CENTER_VERTICAL | wxRIGHT, wxOSX ? 15 : 10);
    ...
    m_hsizer->Add(m_right_sizer, 1, wxEXPAND | wxLEFT, 5);

    // page buttons  
    m_page_buttons_scrolledwin = new wxScrolledWindow(page_parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL);
    m_page_buttons_scrolledwin->SetMinSize(AnkerSize(-1,45));
    //m_page_buttons_scrolledwin->SetMaxSize(wxSize(scrollWidth, -1));
    //m_page_buttons_scrolledwin->SetScrollbars(1, 20, 1, 50);
    m_page_buttons_scrolledwin->SetScrollRate(10, 20);

    m_page_buttons_sizer = new wxBoxSizer(wxHORIZONTAL);

    m_page_buttons_scrolledwin->SetSizer(m_page_buttons_sizer);

    m_right_sizer->Add(m_page_buttons_scrolledwin, 0, wxEXPAND , 5);

    m_page_view = new wxScrolledWindow(page_parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL);
    m_page_view->SetBackgroundColour(wxColour(51, 52, 56));

    m_page_sizer = new wxBoxSizer(wxVERTICAL);
    m_page_view->SetSizer(m_page_sizer);
    m_page_view->SetScrollbars(1, 20, 1, 2);
    m_right_sizer->Add(m_page_view, 1, wxEXPAND | wxLEFT, 0);
    ...
    // modify by dhf, to change page UI
    ...
    timer.Bind(wxEVT_TIMER, [this](wxTimerEvent&) {
        ...
        if (wxGetApp().mainframe) {
            ...
            ANKER_LOG_DEBUG << "  force update,"
                << "  tab:" << this->GetSize().x << "," << this->GetSize().y << " -> " << sz.x - 0 << "," << sz.y
                << "  view:" << m_page_view->GetSize().x << "," << m_page_view->GetSize().y << " -> " << sz.x - 10 << "," << sz.y;

            this->SetMinSize(wxSize(sz.x - 0, sz.y));
            this->SetSize(wxSize(sz.x - 0, sz.y));
            m_page_view->SetMinSize(wxSize(sz.x - 10, sz.y));
            m_page_view->SetSize(wxSize(sz.x - 10, sz.y));

            m_active_page->update_visibility(m_mode);
...
void AnkerTab::add_scaled_button(wxWindow* parent,
                            ScalableButton** btn,
                            ...
                            long style /*= wxBU_EXACTFIT | wxNO_BORDER*/)
{
    *btn = new ScalableButton(parent, wxID_ANY, icon_name, label, wxDefaultSize, wxDefaultPosition, style);
    (ScalableButton*)(*btn)->SetBackgroundColour(wxColour(51, 52, 56));
    m_scaled_buttons.push_back(*btn);
}

void AnkerTab::add_scaled_bitmap(wxWindow* parent,
                            ScalableBitmap& bmp,
                            const std::string& icon_name)
{
...
void AnkerTab::create_search_edit(wxWindow* parent)
{
...

    {
       ...
       //// m_popupWidget->AddItem(gSearchMap);
        ...
        m_popupWidget->SetMaxSize(AnkerSize(276, 368));
        m_popupWidget->SetMinSize(AnkerSize(276, 368));
        m_popupWidget->SetSize(AnkerSize(276, 368));
        ...
        m_pSearchEdit->SetMinSize(AnkerSize(276, 24));
        m_pSearchEdit->SetMaxSize(AnkerSize(276, 24));
        ...
        pSearchIcon->SetMaxSize(wxSize(20, 20));
        ...
        m_pSearchTextCtrl->SetMaxSize(wxSize(270, 24));
        ...
        m_pSearchTextCtrl->Bind(wxEVT_TEXT, [this](wxCommandEvent& event) {
            ...
            if (m_popupWidget)
            {
                ...
                childPos.y = childPos.y + m_pSearchEdit->GetMaxHeight() + 4;
        ...
        pSearchHSizer->Add(m_pSearchTextCtrl, wxALIGN_CENTER_VERTICAL | wxALIGN_CENTER_HORIZONTAL | wxALL, wxALIGN_CENTER_VERTICAL | wxALIGN_CENTER_HORIZONTAL | wxALL, 0);
        pSearchHSizer->AddSpacer(4);
    }
}
...
void AnkerTab::load_initial_data()
{
    ...
    ANKER_LOG_INFO << "tab set m_config=" << m_config;
...
Slic3r::GUI::AnkerPageShp AnkerTab::add_options_page(const wxString& title, const std::string& icon, bool is_extruder_pages /*= false*/)
{
    ...
    // Initialize the page.
    AnkerPageShp page(new AnkerPage(m_page_view, title, icon_idx));
...
wxString AnkerTab::translate_category(const wxString& title, Preset::Type preset_type)
{
...
//    wxWindowUpdateLocker noUpdates(this);
    ...
    //
    // In case the AnkerTab was reparented from the MainFrame to the floating dialog,
    ...
    // and if the AnkerTab was later reparented back to MainFrame, the tooltip was displayed
...
void AnkerTab::update_label_colours()
{
    m_default_text_clr = wxGetApp().get_label_clr_default();
    ...
    for (const auto& opt : m_options_list)
    {
        ...
        if (AnkerOptionsGroup::is_option_without_field(opt.first)) {
            if (AnkerLine* line = get_line(opt.first))
...
// modify by dhf for optiongroup line style 
void AnkerTab::decorate()
{
    for (const auto& opt : m_options_list)
    {
        Field*      field = nullptr;
        ...
        wxColour * color = m_is_default_preset ? &m_default_text_clr : &m_sys_label_clr;
        ...
        // update line icon here
       static wxColour AnkerLableColor = wxColour(173, 174, 175);
        if (option_without_field) {
            if (AnkerLine* line = get_line(opt.first)) {
                ...
                line->set_label_colour(/*color*/&AnkerLableColor);
            }
            continue;
        }
        ...
        field->set_label_colour(&AnkerLableColor);
    }
...
void AnkerTab::update_changed_ui()
{
    ...
    if (m_type == Preset::TYPE_PRINTER) {
        ...
        if (static_cast<AnkerTabPrinter*>(this)->m_printer_technology == ptFFF) {
            AnkerTabPrinter* tab = static_cast<AnkerTabPrinter*>(this);
...
void AnkerTab::init_options_list()
{
...
template<class T>
void add_correct_opts_to_options_list(const std::string &opt_key, std::map<std::string, int>& map, AnkerTab *tab, const int& value)
{
...
void AnkerTab::emplace_option(const std::string& opt_key, bool respect_vec_values/* = false*/)
{
...
void AnkerTabPrinter::init_options_list()
{
    AnkerTab::init_options_list();

    if (m_printer_technology == ptFFF)
        m_options_list.emplace("extruders_count", m_opt_status_value);
}

void AnkerTab::get_sys_and_mod_flags(const std::string& opt_key, bool& sys_page, bool& modified_page)
{
...
// modify by dhf, to change page UI
void AnkerTab::update_changed_tree_ui()
{
    ...
    auto selection = translate_category(m_active_page->title(), m_type);
    for (auto it = m_page_buttons.begin(); it != m_page_buttons.end(); ++it) {
     auto title = (*it)->GetText();
...
void AnkerTab::update_undo_buttons()
{
    m_undo_btn->        SetBitmap_(m_is_modified_values ? m_bmp_value_revert.name(): m_bmp_empty.name());
...
void AnkerTab::on_roll_back_value(const bool to_sys /*= true*/)
{
    ...
    for (auto group : m_active_page->m_optgroups) {
        ...
        if (group->title == "Profile dependencies") {
            // "compatible_printers" option doesn't exists in Printer Settimgs AnkerTab
...
void AnkerTab::load_config(const DynamicPrintConfig& config)
{
    bool modified = 0;
...
void AnkerTab::reload_config()
{
    if (m_active_page)
        m_active_page->reload_config();
}

void AnkerTab::update_mode()
{
...
void AnkerTab::update_mode_markers()
{
    // update mode for ModeSizer
    if (m_mode_sizer)
...
void AnkerTab::update_visibility()
{
...
Field* AnkerTab::get_field(const t_config_option_key& opt_key, int opt_index/* = -1*/) const
{
    return m_active_page ? m_active_page->get_field(opt_key, opt_index) : nullptr;
}

AnkerLine* AnkerTab::get_line(const t_config_option_key& opt_key)
{
    return m_active_page ? m_active_page->get_line(opt_key) : nullptr;
}

std::pair<Anker_OG_CustomCtrl*, bool*> AnkerTab::get_custom_ctrl_with_blinking_ptr(const t_config_option_key& opt_key, int opt_index/* = -1*/)
{
    if (!m_active_page)
        return {nullptr, nullptr};

    std::pair<Anker_OG_CustomCtrl*, bool*> ret = {nullptr, nullptr};
...
Field* AnkerTab::get_field(const t_config_option_key& opt_key, AnkerPage** selected_page, int opt_index/* = -1*/)
{
    Field* field = nullptr;
    for (auto page : m_pages) {
...
void AnkerTab::toggle_option(const std::string& opt_key, bool toggle, int opt_index/* = -1*/)
{
    if (!m_active_page)
        return;
...
void AnkerTab::load_key_value(const std::string& opt_key, const boost::any& value, bool saved_value /*= false*/)
{
...
void AnkerTab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    // This code comment was add by allen for ankerCfgDlg, the configuration panel on the right side  needs attention
  /*  AnkerConfigOptionsGroup* og_freq_chng_params = wxGetApp().sidebar().og_freq_chng_params(is_fff);
...
void AnkerTab::update_wiping_button_visibility() {
...
// mofiy by dhf, to change page UI
void AnkerTab::activate_option(const std::string& opt_key, const wxString& category)
{
    wxString page_title = translate_category(category, m_type);

    // We should to activate a tab with searched option, if it doesn't.
    // And do it before finding of the cur_item to avoid a case when AnkerTab isn't activated jet and all treeItems are invisible
    
    // add by allen do not use this function,use the function of switchAnkerTab
    // wxGetApp().mainframe->selectAnkerTab(this);
    if (wxGetApp().mainframe->m_ankerCfgDlg) {
        wxGetApp().mainframe->m_ankerCfgDlg->switchAnkerTab(m_type);
        wxGetApp().mainframe->m_ankerCfgDlg->updateAnkerTabComBoxHighlight(m_type);
    }

    std::cout<< "   ==activate_option:"<< page_title << " >> " << opt_key << std::endl;
    //change_sel_page(page_title);
    change_page(page_title);
...
void AnkerTab::apply_config_from_cache()
{
    bool was_applied = false;
    // check and apply extruders count for printer preset
    if (m_type == Preset::TYPE_PRINTER)
        was_applied = static_cast<AnkerTabPrinter*>(this)->apply_extruder_cnt_from_cache();
...
void AnkerTab::on_presets_changed()
{
    if (wxGetApp().plater() == nullptr)
        return;
    ...
    for (auto t: m_dependent_tabs)
    {
        ...
        AnkerTab* tab = (AnkerTab*)wxGetApp().getAnkerTab(t);
...
void AnkerTab::build_preset_description_line(AnkerConfigOptionsGroup* optgroup)
{
    ...
    auto detach_preset_btn = [this](wxWindow* parent) {
        ...
        btn->SetFont(ANKER_FONT_NO_1 /*Slic3r::GUI::wxGetApp().normal_font() */ );
...
void AnkerTab::update_preset_description_line()
{
...
void AnkerTab::update_frequently_changed_parameters()
{
    const bool is_fff = supports_printer_technology(ptFFF);
...
void AnkerTabPrint::build()
{
    ANKER_LOG_INFO << "AnkerTabPrint build ....";
    ...
    auto page = add_options_page(L("Layers and perimeters"), "layers");
        ...
        // modify by dhf for AnkerOptionGroup
        //optgroup->append_line(line);
        ...
        {
            AnkerOption option = optgroup->get_option("top_solid_layers");
            option.opt.width = 8;
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("bottom_solid_layers");
            option.opt.width = 8;
            line.append_option(option);
        }
        optgroup->append_line(line);
        ...
        {
            AnkerOption option = optgroup->get_option("top_solid_min_thickness");
            option.opt.width = 8;
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("bottom_solid_min_thickness");
            option.opt.width = 8;
            line.append_option(option);
        }
        optgroup->append_line(line);
        ...
        optgroup->set_opt_width(Field::def_width_8());
    ...
    page = add_options_page(L("Output options"), "output+page_white");
        ...
        {
            AnkerOption option = optgroup->get_option("extruder_clearance_radius");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("extruder_clearance_height");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        optgroup->append_line(line);
        ...
        AnkerOption option = optgroup->get_option("output_filename_format");
        ...
        // modify by dhf to hide description line
        //line = { "", "" };
        //line.full_width = 1;
        //line.widget = [this](wxWindow* parent) {
...
void AnkerTabPrint::update_description_lines()
{
    AnkerTab::update_description_lines();
...
void AnkerTabPrint::toggle_options()
{
    if (!m_active_page) return;
...
void AnkerTabPrint::clear_pages()
{
    AnkerTab::clear_pages();
...
bool AnkerTab::validate_custom_gcode(const wxString& title, const std::string& gcode)
{
...
static void validate_custom_gcode_cb(AnkerTab* tab, const wxString& title, const t_config_option_key& opt_key, const boost::any& value) {
    tab->validate_custom_gcodes_was_shown = !AnkerTab::validate_custom_gcode(title, boost::any_cast<std::string>(value));
    tab->update_dirty();
    tab->on_value_change(opt_key, value);
}

void AnkerTabFilament::create_line_with_near_label_widget(AnkerConfigOptionsGroupShp optgroup, const std::string& opt_key, int opt_index/* = 0*/)
{
    AnkerLine line {"",""};
    if (opt_key == "filament_retract_lift_above" || opt_key == "filament_retract_lift_below") {
        AnkerOption opt = optgroup->get_option(opt_key);

        opt.opt.label = opt.opt.full_label;
        line = optgroup->create_single_option_line(opt);
    }
    else {
        AnkerOption option = optgroup->get_option(opt_key);
        if (optgroup->get_opt_width() > 0)
            option.opt.width = optgroup->get_opt_width();
        line = optgroup->create_single_option_line(option);
    }

    line.near_label_widget = [this, optgroup_wk = AnkerConfigOptionsGroupWkp(optgroup), opt_key, opt_index](wxWindow* parent) {
        ...
        AnkerCheckBox* check_box = nullptr;
		check_box = new AnkerCheckBox(parent,
        ...
        check_box->Bind(wxCUSTOMEVT_ANKER_CHECKBOX_CLICKED, [optgroup_wk, opt_key, opt_index, check_box](wxCommandEvent& evt) {
            if (!check_box)
                return;

            //const bool is_checked = evt.IsChecked();
            const bool is_checked = check_box->GetValue();
        ...
        if (opt_key == "idle_temperature")
            ANKER_LOG_INFO << "====create chkbox," << "  chkbox:" << check_box;
...
void AnkerTabFilament::update_line_with_near_label_widget(AnkerConfigOptionsGroupShp optgroup, const std::string& opt_key_ref, int opt_index/* = 0*/, bool is_checked/* = true*/)
{
    std::string opt_key = opt_key_ref;

    if (optgroup == nullptr) {
        ANKER_LOG_ERROR << "optgroup null, key=" << opt_key;
        return;
    }

    if (m_config == nullptr) {
        ANKER_LOG_ERROR << "m_config null, key=" << opt_key;
        return;
    }

    AnkerCheckBox* chkBox = m_overrides_options[opt_key];
    if (!chkBox) {
            ANKER_LOG_INFO << "checkbox null, key="<< opt_key;
        return;
    }

    if (opt_key == "idle_temperature")
        ANKER_LOG_INFO << " opt_key:" << opt_key << " opt_index:" << opt_index << " ischeck:" << opt_index << "  this:" << this << "  chkbox:" << chkBox<< "  m_config:"<< m_config;

    if (chkBox)
        chkBox->Enable(is_checked);

    ConfigOption* option = m_config->option(opt_key);
    if (option) {
        is_checked &= !option->is_nil();
        if (chkBox)
            chkBox->SetValue(is_checked);
    }
    else {
        ANKER_LOG_ERROR << "option null, key=" << opt_key;
        return;
    }
    ...
    if (field != nullptr)
        ...
        ANKER_LOG_ERROR << "field null, key=" << opt_key;
        return;
    }

    if (opt_key == "idle_temperature")
        ANKER_LOG_INFO << "updated done, opt_key ="<< opt_key;
}

void AnkerTabFilament::add_filament_overrides_page()
{
    AnkerPageShp page = add_options_page(L("Filament Overrides"), "wrench");
    AnkerConfigOptionsGroupShp optgroup = page->new_optgroup(L("Retraction"));
...
void AnkerTabFilament::update_filament_overrides_page()
{
    if (!m_active_page || m_active_page->title() != "Filament Overrides")
        return;
    AnkerPage* page = m_active_page;

    const auto og_it = std::find_if(page->m_optgroups.begin(), page->m_optgroups.end(), [](const AnkerConfigOptionsGroupShp og) { return og->title == "Retraction"; });
    if (og_it == page->m_optgroups.end())
        return;
    AnkerConfigOptionsGroupShp optgroup = *og_it;
...
void AnkerTabFilament::build()
{
    ANKER_LOG_INFO << "AnkerTabFilament build ...." ;
    ...
    auto page = add_options_page(L("Filament"), "spool");
        ...
        AnkerLine line = { L("Nozzle"), "" };
        {
            AnkerOption option = optgroup->get_option("first_layer_temperature");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("temperature");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        optgroup->append_line(line);
        ...
        line = { L("Bed"), "" };
        {
            AnkerOption option = optgroup->get_option("first_layer_bed_temperature");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("bed_temperature");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        optgroup->append_line(line);
    ...
    page = add_options_page(L("Cooling"), "cooling");
        ...
        {
            AnkerOption option = optgroup->get_option("min_fan_speed");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        {
            AnkerOption option = optgroup->get_option("max_fan_speed");
            option.opt.width = Field::def_width_8();
            line.append_option(option);
        }
        ...
        optgroup = page->new_optgroup(L("Cooling thresholds"), 40);
        optgroup->set_opt_width(Field::def_width_8());
    ...
    page = add_options_page(L("Advanced"), "wrench");
        ...
        AnkerOption option = optgroup->get_option("filament_type");
        ...
        create_line_with_widget(optgroup.get(), "filament_ramming_parameters", "", [this](wxWindow* parent) {
            ...
            ramming_dialog_btn->SetFont(ANKER_FONT_NO_1 /*Slic3r::GUI::wxGetApp().normal_font()*/ );
...
void AnkerTabFilament::update_volumetric_flow_preset_hints()
{
    wxString text;
    try {
...
void AnkerTabFilament::update_description_lines()
{
    AnkerTab::update_description_lines();
...
void AnkerTabFilament::toggle_options()
{
    if (!m_active_page)
        return;
    ...
    if (m_active_page->title() == "Filament") {
        ...
        const auto og_it = std::find_if(page->m_optgroups.begin(), page->m_optgroups.end(), [](const AnkerConfigOptionsGroupShp og) { return og->title == "Temperature"; });
        if (og_it != page->m_optgroups.end())
            update_line_with_near_label_widget(*og_it, "idle_temperature");
    }
}

void AnkerTabFilament::update()
{
    if (m_preset_bundle->printers.get_selected_preset().printer_technology() == ptSLA)
        return; // ys_FIXME
...
void AnkerTabFilament::msw_rescale()
{
    for (const auto& over_opt : m_overrides_options)
        if (wxWindow* win = over_opt.second; win != nullptr)
            win->SetInitialSize(win->GetBestSize());

    AnkerTab::msw_rescale();
}

wxSizer* AnkerTab::description_line_widget(wxWindow* parent, AnkerogStaticText* *StaticText, wxString text /*= wxEmptyString*/)
{
    *StaticText = new AnkerogStaticText(parent, text);
//	auto font = (new wxSystemSettings)->GetFont(wxSYS_DEFAULT_GUI_FONT);
    (*StaticText)->SetForegroundColour(wxColour(173, 174, 175));
    (*StaticText)->SetFont(ANKER_FONT_NO_1/*wxGetApp().normal_font()*/);
...
bool AnkerTab::saved_preset_is_dirty() const { return m_presets->saved_is_dirty(); }
void AnkerTab::update_saved_preset_from_current_preset() { m_presets->update_saved_preset_from_current_preset(); }
bool AnkerTab::current_preset_is_dirty() const { return m_presets->current_is_dirty(); }

void AnkerTabPrinter::build()
{
    ANKER_LOG_INFO << "AnkerTabPrinter build ....";
...
void AnkerTabPrinter::build_print_host_upload_group(AnkerPage* page)
{
    AnkerConfigOptionsGroupShp optgroup = page->new_optgroup(L("Print Host upload"));
    ...
    wxString description_line_text = _L(""
        ...
        "into M5 FDM Control/physical_printer directory.");
...
void AnkerTabPrinter::build_fff()
{
    if (!m_pages.empty())
        m_pages.resize(0);
    ...
    auto page = add_options_page(L("General"), "printer");
        AnkerConfigOptionsGroupShp optgroup = page->new_optgroup(L("Size and coordinates"));
        ...
        optgroup->m_on_change = [this, optgroup_wk = AnkerConfigOptionsGroupWkp(optgroup)](t_config_option_key opt_key, boost::any value) {
...
void AnkerTabPrinter::build_sla()
{
    if (!m_pages.empty())
        m_pages.resize(0);
    auto page = add_options_page(L("General"), "printer");
    ...
    AnkerLine line = { option.opt.full_label, "" };
    line.append_option(option);
    ...
    line = AnkerLine{ m_config->def()->get("relative_correction")->full_label, "" };
    for (auto& axis : { "X", "Y", "Z" }) {
...
void AnkerTabPrinter::extruders_count_changed(size_t extruders_count)
{
...
void AnkerTabPrinter::append_option_line(AnkerConfigOptionsGroupShp optgroup, const std::string opt_key)
{
    auto option = optgroup->get_option(opt_key, 0);
    auto line = AnkerLine{ option.opt.full_label, "" };
    line.append_option(option);
...
AnkerPageShp AnkerTabPrinter::build_kinematics_page()
{
    ...
    if (m_use_silent_mode) {
        ...
        auto option = AnkerOption(def, "full_power_legend");
        line.append_option(option);
        ...
        option = AnkerOption(def, "silent_legend");
        line.append_option(option);
...
void AnkerTabPrinter::build_extruder_pages(size_t n_before_extruders)
{
...
// hide this option group
        ...
        auto reset_to_filament_color = [this, extruder_idx](wxWindow*parent) {
            ScalableButton* btn = new ScalableButton(parent, wxID_ANY, "undo", _L("Reset to Filament Color"),
                                                     wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
            btn->SetFont(ANKER_FONT_NO_1 /*wxGetApp().normal_font()*/);
        ...
        AnkerLine line = optgroup->create_single_option_line("extruder_colour", "", extruder_idx);
 ...
 * */
void AnkerTabPrinter::build_unregular_pages(bool from_initial_build/* = false*/)
{
...
void AnkerTabPrinter::update_pages()
{
    ...
    // build AnkerTab according to the technology, if it's not exist jet OR
    // set m_pages_(technology after changing) to m_pages
    // m_printer_technology will be set by AnkerTab::load_current_preset()
...
void AnkerTabPrinter::reload_config()
{
    AnkerTab::reload_config();
...
void AnkerTabPrinter::activate_selected_page(std::function<void()> throw_if_canceled)
{
    AnkerTab::activate_selected_page(throw_if_canceled);
...
void AnkerTabPrinter::clear_pages()
{
    AnkerTab::clear_pages();
...
void AnkerTabPrinter::toggle_options()
{
...
void AnkerTabPrinter::update_sla()
{ ; }

void AnkerTab::update_ui_items_related_on_parent_preset(const Preset* selected_preset_parent)
{
...
void AnkerTab::load_current_preset()
{
    const Preset& preset = m_presets->get_edited_preset();
...
#endif
    {
        // checking out if this AnkerTab exists till this moment
        if (!wxGetApp().checkedAnkerTab(this))
            return;
        update_tab_ui();
        ...
        if (m_type == Slic3r::Preset::TYPE_PRINTER) {
            ...
            if (printer_technology != static_cast<AnkerTabPrinter*>(this)->m_printer_technology)
            {
                ...
                // and as a result a function select_active_page() is called fron AnkerTab::OnActive()
                // But we don't need it. So, to avoid activation of the page, set m_active_page to NULL 
                // till unusable Tabs will be deleted
                AnkerPage* tmp_page = m_active_page;
                m_active_page = nullptr;
                for (auto tab : wxGetApp().ankerTabsList) {
                    if (tab->type() == Preset::TYPE_PRINTER) { // Printer tab is shown every time
                        int cur_selection = wxGetApp().ankerTabPanel()->GetSelection();
                        if (cur_selection != 0)
                            wxGetApp().ankerTabPanel()->SetSelection(wxGetApp().ankerTabPanel()->GetPageCount() - 1);
                        continue;
                    }
                    if (tab->supports_printer_technology(printer_technology))
                    {
                        ...
                        if (!wxGetApp().tabs_as_menu()) {
                            ...
                            dynamic_cast<Notebook*>(wxGetApp().ankerTabPanel())->InsertPage(wxGetApp().ankerTabPanel()->FindPage(this), tab, tab->title(), bmp_name);
                        }
                        else
#endif
                            wxGetApp().ankerTabPanel()->InsertPage(wxGetApp().ankerTabPanel()->FindPage(this), tab, tab->title());
                        #ifdef __linux__ // the tabs apparently need to be explicitly shown on Linux (pull request #1563)
                            int page_id = wxGetApp().ankerTabPanel()->FindPage(tab);
                            wxGetApp().ankerTabPanel()->GetPage(page_id)->Show(true);
                        #endif // __linux__
                    }
                    else {
                        int page_id = wxGetApp().ankerTabPanel()->FindPage(tab);
                        wxGetApp().ankerTabPanel()->GetPage(page_id)->Show(false);
                        wxGetApp().ankerTabPanel()->RemovePage(page_id);
                    }
                }
                static_cast<AnkerTabPrinter*>(this)->m_printer_technology = printer_technology;
                m_active_page = tmp_page;
#ifdef _MSW_DARK_MODE
                if (!wxGetApp().tabs_as_menu())
                    dynamic_cast<Notebook*>(wxGetApp().ankerTabPanel())->SetPageImage(wxGetApp().ankerTabPanel()->FindPage(this), printer_technology == ptFFF ? "printer" : "sla_printer");
#endif
            }
            on_presets_changed();
            if (printer_technology == ptFFF) {
                static_cast<AnkerTabPrinter*>(this)->m_initial_extruders_count = static_cast<const ConfigOptionFloats*>(m_presets->get_selected_preset().config.option("nozzle_diameter"))->values.size(); //static_cast<AnkerTabPrinter*>(this)->m_extruders_count;
                const Preset* parent_preset = m_presets->get_selected_preset_parent();
                static_cast<AnkerTabPrinter*>(this)->m_sys_extruders_count = parent_preset == nullptr ? 0 :
                    static_cast<const ConfigOptionFloats*>(parent_preset->config.option("nozzle_diameter"))->values.size();
            }
        }
        else {
...
// add by dhf, to change page UI
void AnkerTab::rebuild_page_buttons()
{
    ...
    clear_page_buttons();
    // // std::cout << "  ***********************AnkerTab::rebuild_page_tabs m_pages size:" << m_pages.size() << "    tab:" << this->title() << std::endl;
    for (auto p : m_pages)
    {
       // // // std::cout<< "   ==rebuild_page_tabs, add page: " << p->title() <<std::endl;
        if (!p->get_show())
            continue;

        wxString label = translate_category(p->title(), m_type);// p->title();
        AnkerBtn* button = new AnkerBtn(m_page_buttons_scrolledwin, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE);
        // button->SetMinSize(wxSize(-1, 25));
         // button->SetMaxSize(wxSize(252, 24));
       // button->SetLabelAlignment(wxALIGN_CENTER, wxALIGN_CENTER);
        button->SetFont(/*Font*/ANKER_FONT_NO_1);
        ...
        button->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& event) {
            ...
            if (pbtn)
            {
                change_page(pbtn->GetText());
            }
            });

        m_page_buttons_sizer->Add(button, 0, wxALL, 0);
        m_page_buttons.emplace_back(button);
    }

    if (m_page_buttons.size()) {
        change_page(m_page_buttons[0]->GetText());
    }

    // allow activate page before selection of a page 
    m_disable_tree_sel_changed_event = false;
}


void AnkerTab::clear_page_buttons()
{
    for (auto it = m_page_buttons.begin(); it != m_page_buttons.end(); ) {
        if (*it)
            delete* it;
        it = m_page_buttons.erase(it);
    }
}

void AnkerTab::update_btns_enabling()
{
...
void AnkerTab::select_preset(std::string preset_name, bool delete_current /*=false*/, const std::string& last_selected_ph_printer_name/* =""*/)
{
    ...
    if (canceled) {
        ...
        // Initialize the UI from the current preset.
        if (printer_tab)
            static_cast<AnkerTabPrinter*>(this)->update_pages();
        ...
        if (printer_tab && wxGetApp().mainframe->m_ankerCfgDlg) {
            // mod by allen according to the product requirements, when switching machine models and consumables presets, 
            // the default printing parameter package is set to Normal mode.
...
bool AnkerTab::bPrintPresetNeedHide(Preset printPreset) {
    ...
    for (auto inValidIter : inValidFilamentPresetOfPrinerAndPrint) {
        if (inValidIter.first == printPreset.name) {
            auto findInvalidIter = std::find(inValidIter.second.begin(), inValidIter.second.end(), strSelectedFilament);
            if (findInvalidIter != inValidIter.second.end()) {
...
void AnkerTab::selectNormalPrintPreset() {
    const std::deque<Preset>& presets = wxGetApp().preset_bundle->prints.get_presets();
    size_t defaultPresetsNum = wxGetApp().preset_bundle->prints.num_default_presets();
    for (size_t i = presets.front().is_visible ? 0 : defaultPresetsNum; i < presets.size(); ++i) {
        ...
        if (bPrintPresetNeedHide(preset)) {
            continue;
        }
...
bool AnkerTab::may_discard_current_dirty_preset(PresetCollection* presets /*= nullptr*/, const std::string& new_printer_name /*= ""*/)
{
    ...
    else if (dlg.transfer_changes()) // move selected changes
    {
        ...
        if (m_type == presets->type()) // move changes for the current preset from this tab
        {
            if (m_type == Preset::TYPE_PRINTER) {
                ...
                if (it != selected_options.end()) {
                    ...
                    static_cast<AnkerTabPrinter*>(this)->cache_extruder_cnt();
                }
            }
        ...
        else  // add by allen for ankerCfgDlg
            wxGetApp().getAnkerTab(presets->type())->cache_config_diff(selected_options);
    }

    return true;
}

bool AnkerTab::AnkerMayDiscardCurDirtyPreset(PresetCollection* presets/* = nullptr*/, const std::string& new_printer_name/* = ""*/) {
    if (presets == nullptr) presets = m_presets;
...
void AnkerTab::update_description_lines()
{
    if (m_active_page && m_active_page->title() == "Dependencies" && m_parent_preset_description_line)
        update_preset_description_line();
}

void AnkerTab::activate_selected_page(std::function<void()> throw_if_canceled)
{
    if (!m_active_page)
        return;
...
std::string AnkerTab::get_preset_type_str(Preset::Type type)
{
    std::string str = "";
    switch (type)
    {
...
// modify by dhf, to change page UI
bool AnkerTab::change_sel_page(wxString page_tab_text)
{
    ANKER_LOG_INFO<<"change to page:"<< get_preset_type_str(m_type)<<"[" << page_tab_text<<"]";
...
//#endif
    ...
    AnkerPage* page = nullptr;
    const auto selection = page_tab_text;
    for (auto p : m_pages) {
    ...
    catch (const AnkerUIBuildCanceled&) {
        if (m_active_page)
            m_active_page->clear();
        return true;
    }
...
void AnkerTab::OnKeyDown(wxKeyEvent& event)
{
...
void AnkerTab::compare_preset()
{
    wxGetApp().mainframe->diff_dialog.show(m_type);
}

void AnkerTab::transfer_options(const std::string &name_from, const std::string &name_to, std::vector<std::string> options)
{
    if (options.empty())
        return;
    ...
    if (m_type == Preset::TYPE_PRINTER) {
         ...
         if (it != options.end()) {
             ...
             options.erase(it);
             // cache the extruders count
             static_cast<AnkerTabPrinter*>(this)->cache_extruder_cnt(&preset_from->config);
         }
    }
...
void AnkerTab::save_preset(std::string name /*= ""*/, bool detach)
{
    // since buttons(and choices too) don't get focus on Mac, we set focus manually
    // to the treectrl so that the EVT_* events are fired for the input field having
    // focus currently.is there anything better than this ?
//!	m_treectrl->OnSetFocus();
    ANKER_LOG_INFO << "save_preset start. save name:"<<name;
    ...
    if (name.empty()) {
        ...
        AnkerSavePresetDialog dlg(m_parent, { m_type }, detach ? _u8L("Detached") : "", from_template);
        if (dlg.ShowModal() != wxID_OK)
            return;
        name = dlg.get_name();
        if (from_template)
    ...
    if (m_type == Preset::TYPE_PRINTER)
        static_cast<AnkerTabPrinter*>(this)->m_initial_extruders_count = static_cast<AnkerTabPrinter*>(this)->m_extruders_count;
...

    {
        ...
        case Preset::TYPE_SLA_PRINT:
            ...
            break;
        case Preset::TYPE_PRINTER:
            if (static_cast<const AnkerTabPrinter*>(this)->m_printer_technology == ptFFF)
        ...
        for (Preset::Type preset_type : dependent)
            wxGetApp().getAnkerTab(preset_type)->update_tab_ui();
    }
	...
	{
		wxCommandEvent evt = wxCommandEvent(wxCUSTOMEVT_ANKER_SAVE_PRESET);
		evt.SetEventObject(this);
		ProcessEvent(evt);
	}
    ANKER_LOG_INFO << "save_preset end";
}

void AnkerTab::rename_preset()
{
...
void AnkerTab::update_show_hide_incompatible_button()
{
...
void AnkerTab::update_ui_from_settings()
{
...
void AnkerTab::create_line_with_widget(AnkerConfigOptionsGroup* optgroup, const std::string& opt_key, const std::string& path, widget_t widget)
{
    AnkerLine line = optgroup->create_single_option_line(opt_key);
    line.widget = widget;
    line.label_path = path;

    // set default undo ui
    line.set_undo_bitmap(&m_bmp_empty);
...
// Return a callback to create a AnkerTab widget to mark the preferences as compatible / incompatible to the current printer.
wxSizer* AnkerTab::compatible_widget_create(wxWindow* parent, PresetDependencies &deps)
{
	wxSize btSize = wxSize(50, 16);
    ...
    //deps.checkbox = new AnkerCheckBox(parent, wxID_ANY, _(L("All")));
	deps.checkbox = new AnkerCheckBox(parent,
                                        uncheckScaledBitmap.ConvertToImage(),
                                        checkScaledBitmap.ConvertToImage(),
                                        disUncheckScaledBitmap.ConvertToImage(),
                                        disCheckScaledBitmap.ConvertToImage(),
                                        ...
                                        ankerTextColor,
                                        wxID_ANY, wxDefaultPosition, btSize);
    deps.checkbox->SetWindowStyleFlag(wxBORDER_NONE);
    deps.checkbox->SetFont(ANKER_FONT_NO_2);
    ...
    deps.btn = new ScalableButton(parent, wxID_ANY, "printer", format_wxstr(" %s %s", _L("Set"), dots),
                                  wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
    deps.btn->SetFont(/*Slic3r::GUI::wxGetApp().normal_font() */ANKER_FONT_NO_1);
    ...
    deps.checkbox->Bind(wxCUSTOMEVT_ANKER_CHECKBOX_CLICKED, ([this, &deps](wxCommandEvent e)
    {
        deps.btn->Enable(! deps.checkbox->getCheckStatus());
        // All printers have been made compatible with this preset.
        if (deps.checkbox->getCheckStatus())
            this->load_key_value(deps.key_list, std::vector<std::string> {});
        this->get_field(deps.key_condition)->toggle(deps.checkbox->getCheckStatus());
...
void AnkerSubstitutionManager::init(DynamicPrintConfig* config, wxWindow* parent, wxFlexGridSizer* grid_sizer)
{
    m_config = config;
    m_parent = parent;
...
void AnkerSubstitutionManager::validate_lenth()
{
...
bool AnkerSubstitutionManager::is_compatibile_with_ui()
{
...
bool AnkerSubstitutionManager::is_valid_id(int substitution_id, const wxString& message)
{
    const std::vector<std::string>& substitutions = m_config->option<ConfigOptionStrings>("gcode_substitutions")->values;
...
void AnkerSubstitutionManager::create_legend()
{
    ...
    wxColour labelColor = wxColour(173, 174, 175);
    auto legend_sizer = new wxBoxSizer(wxHORIZONTAL); // "Find", "Replace", "Notes"
    wxStaticText* find_label = new wxStaticText(m_parent, wxID_ANY, _L("Find"));
    wxStaticText* replace_Label = new wxStaticText(m_parent, wxID_ANY, _L("Replace with"));
    wxStaticText* note_label = new wxStaticText(m_parent, wxID_ANY, _L("Notes"));
    find_label->SetForegroundColour(labelColor);
    replace_Label->SetForegroundColour(labelColor);
    note_label->SetForegroundColour(labelColor);
    legend_sizer->Add(find_label,         3, wxEXPAND);
    legend_sizer->Add(replace_Label, 3, wxEXPAND);
    legend_sizer->Add(note_label,      2, wxEXPAND);

    m_grid_sizer->Add(legend_sizer, 1, wxEXPAND);
}

// delete substitution_id from substitutions
void AnkerSubstitutionManager::delete_substitution(int substitution_id)
{
    validate_lenth();
...
void AnkerSubstitutionManager::add_substitution( int substitution_id, 
                                            ...
                                            const std::string& notes)
{
    ...
    auto add_text_editor = [substitution_id, top_sizer, this](const wxString& value, int opt_pos, int proportion) {
        ...
        editor->SetFont(/*wxGetApp().normal_font()*/ANKER_FONT_NO_2);
    ...
    //auto chb_regexp = new AnkerCheckBox(m_parent, wxID_ANY, _L("Regular expression"));
    wxString text = _L("Regular expression");
	auto chb_regexp = new AnkerCheckBox(m_parent,
                                        uncheckScaledBitmap.ConvertToImage(),
                                        checkScaledBitmap.ConvertToImage(),
                                        disUncheckScaledBitmap.ConvertToImage(),
                                        disCheckScaledBitmap.ConvertToImage(),
    ...
    chb_regexp->SetMinSize(wxSize(chb_regexp->GetTextExtent(text).GetWidth()+30, 20));
    params_sizer->Add(chb_regexp, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, m_em);

    //auto chb_case_insensitive = new AnkerCheckBox(m_parent, wxID_ANY, _L("Case insensitive"));
    text = _L("Case insensitive");
	auto chb_case_insensitive = new AnkerCheckBox(m_parent,
                                                    uncheckScaledBitmap.ConvertToImage(),
                                                    checkScaledBitmap.ConvertToImage(),
                                                    disUncheckScaledBitmap.ConvertToImage(),
                                                    disCheckScaledBitmap.ConvertToImage(),
    ...
    chb_case_insensitive->SetMinSize(wxSize(chb_regexp->GetTextExtent(text).GetWidth() + 30, 20));
    params_sizer->Add(chb_case_insensitive, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, m_em);

    //auto chb_whole_word = new AnkerCheckBox(m_parent, wxID_ANY, _L("Whole word"));
    text = _L("Whole word");
	auto chb_whole_word = new AnkerCheckBox(m_parent,
                                            uncheckScaledBitmap.ConvertToImage(),
                                            checkScaledBitmap.ConvertToImage(),
                                            disUncheckScaledBitmap.ConvertToImage(),
                                            disCheckScaledBitmap.ConvertToImage(),
    ...
    chb_whole_word->SetMinSize(wxSize(chb_regexp->GetTextExtent(text).GetWidth() + 30, 20));
    params_sizer->Add(chb_whole_word, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, m_em);

    //auto chb_match_single_line = new AnkerCheckBox(m_parent, wxID_ANY, _L("Match single line"));
    text = _L("Match single line");
	auto chb_match_single_line = new AnkerCheckBox(m_parent,
		                                           uncheckScaledBitmap.ConvertToImage(),
		                                           checkScaledBitmap.ConvertToImage(),
		                                           disUncheckScaledBitmap.ConvertToImage(),
		                                           disCheckScaledBitmap.ConvertToImage(),
    ...
    chb_match_single_line->SetMinSize(wxSize(chb_regexp->GetTextExtent(text).GetWidth() + 30, 20));
    chb_match_single_line->Show(regexp);
    params_sizer->Add(chb_match_single_line, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, m_em);

    for (AnkerCheckBox* chb : std::initializer_list<AnkerCheckBox*>{ chb_regexp, chb_case_insensitive, chb_whole_word, chb_match_single_line }) {
        chb->SetFont(ANKER_FONT_NO_1);
        chb->setBgColor(chb->GetParent()->GetBackgroundColour());
        chb->Bind(wxCUSTOMEVT_ANKER_CHECKBOX_CLICKED, [this, substitution_id, chb_regexp, chb_case_insensitive, chb_whole_word, chb_match_single_line](wxCommandEvent e) {
...
void AnkerSubstitutionManager::update_from_config()
{
    if (!m_grid_sizer->IsEmpty())
...
void AnkerSubstitutionManager::delete_all()
{
...
void AnkerSubstitutionManager::edit_substitution(int substitution_id, int opt_pos, const std::string& value)
{
    std::vector<std::string>& substitutions = m_config->option<ConfigOptionStrings>("gcode_substitutions")->values;
...
bool AnkerSubstitutionManager::is_empty_substitutions()
{
    return m_config->option<ConfigOptionStrings>("gcode_substitutions")->values.empty();
}

// Return a callback to create a AnkerTabPrint widget to edit G-code substitutions
wxSizer* AnkerTabPrint::create_manage_substitution_widget(wxWindow* parent)
{
    auto create_btn = [parent](ScalableButton** btn, const wxString& label, const std::string& icon_name) {
        *btn = new ScalableButton(parent, wxID_ANY, icon_name, " " + label + " ", wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
        (*btn)->SetFont(/*wxGetApp().normal_font() */ANKER_FONT_NO_1);
        (*btn)->SetSize((*btn)->GetBestSize());
    };

    ScalableButton* add_substitution_btn;
    create_btn(&add_substitution_btn, _L("Add"), "add_copies");
    add_substitution_btn->Bind(wxEVT_BUTTON, [&](wxCommandEvent e) {
...
// Return a callback to create a AnkerTabPrint widget to edit G-code substitutions
wxSizer* AnkerTabPrint::create_substitutions_widget(wxWindow* parent)
{
...
// Return a callback to create a AnkerTabPrinter widget to edit bed shape
wxSizer* AnkerTabPrinter::create_bed_shape_widget(wxWindow* parent)
{
...
void AnkerTabPrinter::cache_extruder_cnt(const DynamicPrintConfig* config/* = nullptr*/)
{
...
bool AnkerTabPrinter::apply_extruder_cnt_from_cache()
{
...
bool AnkerTab::validate_custom_gcodes()
{
    // add by dhf, to change page UI
    if (!m_active_page)
        return false;
    if (m_type != Preset::TYPE_FILAMENT &&
        (m_type != Preset::TYPE_PRINTER || static_cast<AnkerTabPrinter*>(this)->m_printer_technology != ptFFF))
        return true;
...
void AnkerTab::onAnkerPresetsChanged()
{
    on_presets_changed();
}

void AnkerTabPrinter::update_machine_limits_description(const MachineLimitsUsage usage)
{
	wxString text;
	switch (usage) {
...
void AnkerTab::compatible_widget_reload(PresetDependencies &deps)
{
...
void AnkerTab::fill_icon_descriptions()
{
...
void AnkerTab::set_tooltips_text()
{
...
AnkerPage::AnkerPage(wxWindow* parent, const wxString& title, int iconID) :
        m_parent(parent),
        m_title(title),
        m_iconID(iconID)
{
...
void AnkerPage::reload_config()
{
    for (auto group : m_optgroups)
        group->reload_config();
}

void AnkerPage::update_visibility(ConfigOptionMode mode, bool update_contolls_visibility)
{
    bool ret_val = false;
    for (auto group : m_optgroups) {
...
void AnkerPage::activate(ConfigOptionMode mode, std::function<void()> throw_if_canceled)
{
    // std::cout << "AnkerPage::activate,  " << "      page name:" << this->title() << std::endl;
    for (auto group : m_optgroups) {
...
void AnkerPage::update_visibility(ConfigOptionMode mode)
{
    // ANKER_LOG_INFO << "AnkerPage::activate,  " << "      page name:" << this->title() << std::endl;
    for (auto group : m_optgroups) {
        group->update_visibility(mode);
    }
}

void AnkerPage::clear()
{
    for (auto group : m_optgroups)
        group->clear();
}

void AnkerPage::msw_rescale()
{
    for (auto group : m_optgroups)
        group->msw_rescale();
}

void AnkerPage::sys_color_changed()
{
    for (auto group : m_optgroups)
        group->sys_color_changed();
}

void AnkerPage::refresh()
{
    for (auto group : m_optgroups)
        group->refresh();
}

Field* AnkerPage::get_field(const t_config_option_key& opt_key, int opt_index /*= -1*/) const
{
    Field* field = nullptr;
...
AnkerLine* AnkerPage::get_line(const t_config_option_key& opt_key)
{
    for (auto opt : m_optgroups)
        if (AnkerLine* line = opt->get_line(opt_key))
            return line;
    return nullptr;
}

bool AnkerPage::set_value(const t_config_option_key& opt_key, const boost::any& value) {
    bool changed = false;
...
AnkerConfigOptionsGroupShp AnkerPage::new_optgroup(const wxString& title, int noncommon_label_width /*= -1*/)
{
    //! config_ have to be "right"
    // modify by dhf for Anker style (no static_box) OptionsGroup (AnkerOptionsGroup)
    AnkerConfigOptionsGroupShp optgroup = std::make_shared<AnkerConfigOptionsGroup>(m_parent, title, m_config, true, nullptr, false);
...
#endif
    optgroup->set_config_category_and_type(m_title, static_cast<AnkerTab*>(tab)->type());
    optgroup->m_on_change = [tab](t_config_option_key opt_key, boost::any value) {
        //! This function will be called from OptionGroup.
        //! Using of CallAfter is redundant.
        //! And in some cases it causes update() function to be recalled again
//!        wxTheApp->CallAfter([this, opt_key, value]() {
            static_cast<AnkerTab*>(tab)->update_dirty();
            static_cast<AnkerTab*>(tab)->on_value_change(opt_key, value);
//!        });
    };

    optgroup->m_get_initial_config = [tab]() {
        DynamicPrintConfig config = static_cast<AnkerTab*>(tab)->m_presets->get_selected_preset().config;
        return config;
    };

    optgroup->m_get_sys_config = [tab]() {
        DynamicPrintConfig config = static_cast<AnkerTab*>(tab)->m_presets->get_selected_preset_parent()->config;
        return config;
    };

    optgroup->have_sys_config = [tab]() {
        return static_cast<AnkerTab*>(tab)->m_presets->get_selected_preset_parent() != nullptr;
    };
...
const AnkerConfigOptionsGroupShp AnkerPage::get_optgroup(const wxString& title) const
{
    for (AnkerConfigOptionsGroupShp optgroup : m_optgroups) {
...
void AnkerTabSLAMaterial::build()
{
    ANKER_LOG_INFO << "AnkerTabSLAMaterial build ....";
    ...
    optgroup = page->new_optgroup(L("Corrections"));
    auto line = AnkerLine{ m_config->def()->get("material_correction")->full_label, "" };
    for (auto& axis : { "X", "Y", "Z" }) {
    ...
    optgroup = page->new_optgroup(L("Notes"), 0);
    optgroup->label_width = 0;
    AnkerOption option = optgroup->get_option("material_notes");
    option.opt.full_width = true;
    option.opt.height = 25;//250;
    optgroup->append_single_option_line(option);
...
void AnkerTabSLAMaterial::toggle_options()
{
...
void AnkerTabSLAMaterial::update()
{
...
void AnkerTabSLAPrint::build_sla_support_params(const std::vector<SamePair<std::string>> &prefixes,
...
void AnkerTabSLAPrint::update_description_lines()
{
    AnkerTab::update_description_lines();
...
void AnkerTabSLAPrint::toggle_options()
{
    if (m_active_page)
        m_config_manipulation.toggle_print_sla_options(m_config);
}

void AnkerTabSLAPrint::update()
{
    if (m_preset_bundle->printers.get_selected_preset().printer_technology() == ptFFF)
        return;
...
void AnkerTabSLAPrint::clear_pages()
{
    AnkerTab::clear_pages();
...
ConfigManipulation AnkerTab::get_config_manipulation()
{
...
inline AnkerTabPresetComboBox* AnkerTab::getAnkerTabPresetCombo(const Preset::Type type) {
    
    return wxGetApp().mainframe->m_ankerCfgDlg ? 
        wxGetApp().mainframe->m_ankerCfgDlg->GetAnkerTabPresetCombo(type) : nullptr;
}
