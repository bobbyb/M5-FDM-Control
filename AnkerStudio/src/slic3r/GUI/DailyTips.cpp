...
public:
    DailyTipsDataRenderer(DailyTipsLayout layout, DailyTipsPanel* tip_panel);
...
protected:
    ...
    void render_img_navigator(const ImVec2& start_pos, const ImVec2& size) const;
    void render_text(const ImVec2& start_pos, const ImVec2& size) const;

private:
    DailyTipsPanel* m_tip_panel{ nullptr };
...
DailyTipsDataRenderer::DailyTipsDataRenderer(DailyTipsLayout layout, DailyTipsPanel* tip_panel)
    : m_layout(layout)
    , m_tip_panel(tip_panel)
{
}
   ...
   // ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.1f, 0.3f, 0.6f, 1.0f)); 
    if (ImGui::BeginChild(name.c_str(), size, false, window_flags)) {
        if (m_layout == DailyTipsLayout::Vertical) {
            // The image band's 16:9 height is derived from the panel WIDTH, so on a wide
            // notification it grows until it has eaten nearly the whole panel and the tip
            // text is left with a few unreadable pixels. Cap it so the text always keeps
            // its share of the height.
            const float navigator_height = std::min(9.0f / 16.0f * size.x, 0.55f * size.y);
            ImVec2 navigator_size = ImVec2(size.x, navigator_height);
            render_img_navigator({ 0, 0 }, navigator_size);
            float img_text_gap = ImGui::CalcTextSize("A").y;
            render_text({ 0, navigator_size.y + img_text_gap}, { size.x, size.y - navigator_size.y - img_text_gap });
        }
        if (m_layout == DailyTipsLayout::Horizontal) {
            // Same guard on the other axis: the band's width comes from the panel HEIGHT.
            const float navigator_width = std::min(16.0f / 9.0f * size.y, 0.55f * size.x);
            ImVec2 navigator_size = ImVec2(navigator_width, size.y);
            render_img_navigator({ 0, 0 }, navigator_size);
            float img_text_gap = ImGui::CalcTextSize("A").y;
            render_text({ navigator_size.x + img_text_gap, 0 }, { size.x - navigator_size.x - img_text_gap, size.y });
        }
    }
    ImGui::EndChild();
 //   ImGui::PopStyleColor();
...
void DailyTipsDataRenderer::render_img_navigator(const ImVec2& start_pos, const ImVec2& size) const
{
    ImGuiWrapper& imgui = *wxGetApp().imgui();
    int hint_count = HintDatabase::get_instance().get_valid_hint_count();

    float scale = imgui.get_font_size() / 15.0f;
    ImVec2 button_size = ImVec2(32.0f, 32.0f) * scale;

    ImVec2 prev_btn_pos = ImVec2(0, (size.y - button_size.y)/2.0f);
    ImVec2 next_btn_pos = ImVec2(start_pos.x + size.x - button_size.x - 4 * scale, (size.y - button_size.y) / 2.0f);
    if (hint_count > 1) {
        ImColor text_color = m_is_dark ? ImColor(255, 255, 255, (int)(255 * m_fade_opacity)) : ImColor(38, 46, 48, (int)(255 * m_fade_opacity));
        ...
        // render Preview tip btn
        {
            //ImGui::SetCursorScreenPos(prev_btn_pos);
            ImGui::SetCursorPos(prev_btn_pos);
            ImVec2 btn_screen_pos = ImGui::GetCursorScreenPos();
            std::wstring button_text;
            button_text = ImGui::PrevArrowBtnIcon;
            if (ImGui::IsMouseHoveringRect(btn_screen_pos, btn_screen_pos + button_size, true))
            {
                text_color = ImColor(0, 174, 66, (int)(255 * m_fade_opacity));
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
                    if (m_tip_panel)
                        m_tip_panel->retrieve_data_from_hint_database(HintDataNavigation::Prev);
                }
            }
        ...
        // render next tip icon
        {
            //ImGui::SetCursorScreenPos(next_btn_pos);
            ImGui::SetCursorPos(next_btn_pos);
            ImVec2 btn_screen_pos = ImGui::GetCursorScreenPos();
            std::wstring button_text;
            ...
            if (ImGui::IsMouseHoveringRect(btn_screen_pos, btn_screen_pos + button_size, true))
            {
                ...
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
                    if (m_tip_panel)
                        m_tip_panel->retrieve_data_from_hint_database(HintDataNavigation::Next);
                }
            }
            ImGui::PushStyleColor(ImGuiCol_Text, text_color.Value);
            imgui.button(button_text.c_str());
    ...
    // render image
    int spacing = 10;
    float img_width = next_btn_pos.x - prev_btn_pos.x - button_size.x - spacing * 2;
    ImVec2 img_pos = ImVec2(prev_btn_pos.x + button_size.x + spacing, 0);
    ImVec2 img_size = ImVec2(img_width, size.y);

    // Letterbox the artwork inside the band at its own aspect ratio instead of
    // stretching it to fill: the tip images are ~4:3, so filling a 16:9 band blew the
    // printer picture up and distorted it.
    const GLTexture* img_texture = has_image() ? m_texture : m_placeholder_texture;
    if (img_texture && img_texture->get_width() > 0 && img_texture->get_height() > 0 &&
        img_width > 0.0f && size.y > 0.0f) {
        const float tex_aspect = (float)img_texture->get_width() / (float)img_texture->get_height();
        if (img_width / size.y > tex_aspect)
            img_size = ImVec2(size.y * tex_aspect, size.y);
        else
            img_size = ImVec2(img_width, img_width / tex_aspect);
        img_pos.x += (img_width - img_size.x) * 0.5f;
        img_pos.y += (size.y - img_size.y) * 0.5f;
    }

    ImGui::SetCursorPos(img_pos);
    if (has_image())
        ImGui::Image((ImTextureID)(intptr_t)m_texture->get_id(), /*size*/img_size, ImVec2(0, 0), ImVec2(1, 1), m_is_dark ? ImVec4(0.8, 0.8, 0.8, m_fade_opacity) : ImVec4(1, 1, 1, m_fade_opacity));
    else {
        ImGui::Image((ImTextureID)(intptr_t)m_placeholder_texture->get_id(), /*size*/img_size, ImVec2(0, 0), ImVec2(1, 1), m_is_dark ? ImVec4(0.8, 0.8, 0.8, m_fade_opacity) : ImVec4(1, 1, 1, m_fade_opacity));
    }
}
...
void DailyTipsDataRenderer::render_text(const ImVec2& start_pos, const ImVec2& size) const
{
    ...
    int window_flags = parent_window->Flags;
    window_flags &= ~ImGuiWindowFlags_NoScrollbar;
    std::string name = "##DailyTipsContent" + std::to_string(parent_window->ID);
    ImGui::BeginChild(name.c_str(), size - ImVec2(0,10.0f), false, window_flags);
    ...
    std::string title_line = m_data.title;
    imgui.push_bold_font();
    //imgui.text(title_line);
    imgui.text_wrapped(title_line, size.x - 5);
    imgui.pop_bold_font();
    ImGui::PopStyleColor();


    ImGui::PushStyleColor(ImGuiCol_Text, m_is_dark ? ImVec4(170.0f/255.f, 170.0f / 255.f, 170.0f / 255.f, 0.88f * m_fade_opacity) : ImVec4(38 / 255.0f, 46 / 255.0f, 48 / 255.0f, m_fade_opacity));
    std::string content_lines = m_data.main_text;
    bool is_zh = false;
    ...
    if (!is_zh) {
        ...
        imgui.text_wrapped(content_lines, size.x - 5);
    }
    else {
#if 0
    ...
    // link
    if (!m_data.wiki_url.empty()) {
        ImColor HyperColor = ImColor(96, 211, 96, (int)(255 * m_fade_opacity)).Value;
        ImGui::PushStyleColor(ImGuiCol_Text, HyperColor.Value);
        //imgui.text(m_data.hyper_text);
        imgui.text_wrapped(m_data.hyper_text, size.x - 5);  // label wrap when link text tool long
        ImGui::PopStyleColor();
        //click behavior
...
DailyTipsPanel::DailyTipsPanel(bool can_expand, DailyTipsLayout layout)
    ...
    m_dailytips_renderer(std::make_unique<DailyTipsDataRenderer>(layout, this))
{
    set_header_footer_height();
    m_is_expanded = wxGetApp().app_config->get("show_hints") != "0";
}
...
void DailyTipsPanel::set_header_footer_height()
{
    ImGuiWrapper& imgui = *wxGetApp().imgui();
    float scale = imgui.get_font_size() / 15.0f;
    m_header_height = 30.0f * scale;
    //m_footer_height = 58.0f * scale;
    m_footer_height = 0.0f * scale;
}
...
void DailyTipsPanel::set_size(const ImVec2& size)
{
    ...
    m_width = size.x;
    m_height = size.y;
    m_content_height = m_height - m_footer_height - m_header_height;
}
...
void DailyTipsPanel::render()
{
    ...
    if (m_can_expand) {
        if (m_is_expanded) {
            ...
            m_height = m_header_height + m_footer_height;
        }
    }
    ...
    // title : "tips"  m_header_height
    ImVec2 header_win_size = ImVec2(m_width, m_header_height);
    render_Header(m_pos, header_win_size);


    ImVec2 pos = m_pos  + ImVec2(0, header_win_size.y) ;
    ImGui::SetNextWindowPos(pos);
    ImGui::SetNextWindowSizeConstraints(ImVec2(m_width, m_height), ImVec2(m_width, m_height));
    ...
    if (ImGui::BeginChild((std::string("##DailyTipsPanel") + std::to_string(m_uid)).c_str(), ImVec2(m_width, m_height), false, window_flags)) {
        ...
        if (m_can_expand) {
            if (m_is_expanded) {
                m_dailytips_renderer->render({ pos.x, pos.y }, { m_width, m_content_height });
                render_controller_buttons({ pos.x, pos.y + m_height-m_footer_height}, { m_width, m_footer_height });
            }
            else {
                render_controller_buttons({ pos.x, pos.y }, {m_width, m_footer_height});
            }
        }
        else {
            m_dailytips_renderer->render({ pos.x, pos.y }, { m_width, m_content_height });
            render_controller_buttons({ pos.x, pos.y + m_header_height + m_content_height }, { m_width, m_footer_height });
        }
#else
        m_dailytips_renderer->render({ pos.x, pos.y }, { m_width, m_content_height });
#endif
...
void DailyTipsPanel::init_hints()
{
    HintDatabase::get_instance().reinit();
}

int DailyTipsPanel::get_hint_cnt()
{
    return HintDatabase::get_instance().get_valid_hint_count();
}
...
void DailyTipsPanel::retrieve_data_from_hint_database(HintDataNavigation nav)
{
    ...
    {
        DailyTipsData data{ hint_data->title,
                            hint_data->text,
                            hint_data->documentation_link,
                            hint_data->image,
                            hint_data->follow_text,
                            hint_data->documentation_link_text,
                            hint_data->callback
        };
...
void DailyTipsPanel::expand(bool expand)
{
    ...
    wxGetApp().app_config->set("show_hints", std::to_string(static_cast<int>(expand)));
}
...
void DailyTipsPanel::collapse()
{
    if (!m_can_expand)
        return;
    m_is_expanded = false;
    wxGetApp().app_config->set("show_hints", std::to_string(static_cast<int>(false)));
}
...
void DailyTipsPanel::render_Header(const ImVec2& pos, const ImVec2& size)
{
   ...
   // ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.1f, 0.7f, 0.6f, 1.0f));
    if (ImGui::BeginChild(name.c_str(), size, false, window_flags)) {
        // "tips"
        auto h = ImGui::CalcTextSize("A").y;
        ImVec2 text_pos = pos + ImVec2(0, (size.y - ImGui::CalcTextSize("A").y) / 2);
        ImGui::SetCursorScreenPos(text_pos);
        imgui.push_bold_font();
        ...
        float scale = imgui.get_font_size() / 15.0f;
        int right_pading = 10 * scale;
        ImColor text_color = m_is_dark ? ImColor(255, 255, 255, (int)(255 * m_fade_opacity)) : ImColor(38, 46, 48, (int)(255 * m_fade_opacity));
        ImVec2 button_size = ImVec2(16.0f, 16.0f) * scale;
        ImVec2 btn_pos = pos + ImVec2(size.x - right_pading - button_size.x, (size.y - button_size.y) / 2);
        ImGui::SetCursorScreenPos(btn_pos);
        std::wstring button_text;
        if (m_can_expand) {
            if (m_is_expanded) {
                button_text = ImGui::CollapseArrowIcon;
                if (ImGui::IsMouseHoveringRect(btn_pos, btn_pos + button_size, true))
                {
                    text_color = ImColor(0, 174, 66, (int)(255 * m_fade_opacity));
                    if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) 
                        collapse();
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, text_color.Value);// for icon-font button
        imgui.button(button_text.c_str());
        ImGui::PopStyleColor();
...
void DailyTipsPanel::render_controller_buttons(const ImVec2& pos, const ImVec2& size)
{
    ...
    if (ImGui::BeginChild(name.c_str(), size, false, window_flags)) {
        ...
        m_pages_count = HintDatabase::get_instance().get_valid_hint_count();
        ...
        // prev button
        ImColor button_text_color = m_is_dark ? ImColor(228, 228, 228, (int)(255 * m_fade_opacity)) : ImColor(38, 255, 48, (int)(255 * m_fade_opacity));
