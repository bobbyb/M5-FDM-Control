...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that AnkerStudio.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon AnkerStudio.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "icons")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/icons/AnkerStudio.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%AnkerStudio%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying AnkerStudio icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/icons/%2%AnkerStudio%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            std::string path = GUI::format("%1%/applications/AnkerStudio%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file)) {
                candidate_found = true;
                BOOST_LOG_TRIVIAL(debug) << "AnkerStudio.desktop file installation success.";
                break;
            }
            else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to AnkerStudio.desktop file installation failed. failed path: " << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/AnkerStudio%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/icons/AnkerStudio-gcodeviewer_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%AnkerStudio-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file_viewer = GUI::format(
            "[Desktop Entry]\n"
            "Name=Anker Gcode Viewer%1%\n"
            "GenericName=3D Printing Software\n"
            "Icon=AnkerStudio-gcodeviewer%2%\n"
            ...
            "StartupNotify=false\n", name_suffix, version_suffix, excutable_path);
        std::string desktop_path = GUI::format("%1%/applications/AnkerStudioGcodeViewer%2%.desktop", target_dir_desktop, version_suffix);
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration()
{
    ...
    std::string desktop_file_downloader = GUI::format(
        "[Desktop Entry]\n"
        "Name=AnkerStudio URL Protocol%1%\n"
        "Exec=\"%2%\" --single-instance %%u\n"
        "Terminal=false\n"
        "Type=Application\n"
        "MimeType=x-scheme-handler/ankerstudio;\n"
        "StartupNotify=false\n"
        "NoDisplay=true\n"
        , name_suffix, excutable_path);

    // desktop file for downloader as part of main app
    std::string desktop_path = GUI::format("%1%/applications/AnkerStudioURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default AnkerStudioURLProtocol%1%.desktop x-scheme-handler/ankerstudio", version_suffix);
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = GUI::format("%1%/applications/AnkerStudioURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file_downloader)) {
                app_config->set("desktop_integration_URL_path", path);
                candidate_found = true;
                BOOST_LOG_TRIVIAL(debug) << "AnkerStudioURLProtocol.desktop file installation success.";
                break;
            }
            else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to AnkerStudioURLProtocol.desktop file installation failed. failed path: " << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
    }
