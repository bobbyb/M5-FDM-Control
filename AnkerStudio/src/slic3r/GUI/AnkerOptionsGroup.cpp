...
const t_field& AnkerOptionsGroup::build_field(const AnkerOption& opt) {
    return build_field(opt.opt_id, opt.opt);
}
const t_field& AnkerOptionsGroup::build_field(const t_config_option_key& id) {
	const ConfigOptionDef& opt = m_options.at(id).opt;
    return build_field(id, opt);
}


bool AnkerOptionsGroup::sidetext_is_unit(wxString sideText)
{
    return sideText == _L("g")
        || sideText == _L("s")
        || sideText == _L("°")
        || sideText == _L("°C")
...
const t_field& AnkerOptionsGroup::build_field(const t_config_option_key& id, const ConfigOptionDef& opt) {
    // Check the gui_type field first, fall through
    // is the normal type.

    wxString unit = "";
    if (!(_(opt.sidetext).empty())) {
        unit = sidetext_is_unit(_(opt.sidetext)) ? _(opt.sidetext) : "";
    }
    ...
    case ConfigOptionDef::GUIType::i_enum_open:
        m_fields.emplace(id, AnkerChoiceField::Create<AnkerChoiceField>(this->ctrl_parent(), opt, id));
        break;
    ...
    case ConfigOptionDef::GUIType::one_string:
        if (opt.multiline)
            ...
            m_fields.emplace(id, AnkerTextCtrlField::Create<AnkerTextCtrlField>(this->ctrl_parent(), opt, id, unit));
        break;
    default:
        switch (opt.type) {
			...
			case coBool:
			case coBools:
                m_fields.emplace(id, AnkerCheckBoxField::Create<AnkerCheckBoxField>(this->ctrl_parent(), opt, id));
				break;
			case coInt:
			case coInts:
                //m_fields.emplace(id, SpinCtrl::Create<SpinCtrl>(this->ctrl_parent(), opt, id));
                m_fields.emplace(id, AnkerSpinCtrl::Create<AnkerSpinCtrl>(this->ctrl_parent(), opt, id, unit));
...
AnkerOptionsGroup::AnkerOptionsGroup(	wxWindow* _parent, const wxString& title,
                            bool is_tab_opt /* = false */,
                            AnkerColumn_t extra_clmn /* = nullptr */) :
...
// add by dhf for setting Anker style (no static_box) OptionsGroup 
AnkerOptionsGroup::AnkerOptionsGroup(wxWindow* _parent, const wxString& title,
    bool is_tab_opt /* = false */,
    bool use_static_box /*= false*/,
    AnkerColumn_t extra_clmn /* = nullptr */
    ) :
    m_parent(_parent), title(title),
    m_use_custom_ctrl(is_tab_opt),
    staticbox(use_static_box), extra_column(extra_clmn)
{
}

AnkerOption::AnkerOption(const ConfigOptionDef& _opt, t_config_option_key id) : opt(_opt), opt_id(id)
{
...
wxWindow* AnkerOptionsGroup::ctrl_parent() const
{
...
bool AnkerOptionsGroup::is_legend_line()
{
    // std::cout << " ==--====is_legend_line 1 , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;
	if (m_lines.size() == 1) {
		const std::vector<AnkerOption>& option_set = m_lines.front().get_options();
		return !option_set.empty() && option_set.front().opt.gui_type == ConfigOptionDef::GUIType::legend;
	}
	return false;
}

void AnkerOptionsGroup::set_max_win_width(int max_win_width)
{
    // std::cout << " ==--====set_max_win_width 1 , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;
    if (custom_ctrl)
        custom_ctrl->set_max_win_width(max_win_width);
    // std::cout << " ==--====set_max_win_width 2 , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;
}

void AnkerOptionsGroup::show_field(const t_config_option_key& opt_key, bool show/* = true*/)
{
...
void AnkerOptionsGroup::append_line(const AnkerLine& line)
{
...
void AnkerOptionsGroup::append_separator()
{
    m_lines.emplace_back(AnkerLine());
}

void AnkerOptionsGroup::activate_line(AnkerLine& line)
{
    //std::cout << "AnkerOptionsGroup::activate_line,  " << "      line name:" << line.label << std::endl;
    if (line.is_separator()) {
        // std::cout << "                  in activate_line, 1 , is_separator" << std::endl;
        return;
    }
    ...
    if (!custom_ctrl && m_use_custom_ctrl) {
        wxWindow* parent = is_legend_line || !staticbox ? this->parent() : static_cast<wxWindow*>(this->stb);
        custom_ctrl = new Anker_OG_CustomCtrl(parent, this);
        custom_ctrl->SetBackgroundColour(wxColour(51, 52, 56));
        custom_ctrl->SetMinSize(wxSize(200, 20));
        //custom_ctrl->SetSize(wxSize(parent? parent->GetClientRect().GetWidth()-20:600, -1));
		 //if (is_legend_line)
    ...
    if (label_width != 0) {
        if (custom_ctrl) {
            ...
            if (!line.near_label_widget || !line.label.IsEmpty()) {
                ...
                label->SetFont(/*wxGetApp().normal_font()*/ANKER_FONT_NO_1);
    ...
    for (const AnkerOption& opt : option_set) {
        ...
        if (!custom_ctrl) {
            ...
            if (!option.sidetext.empty() || sidetext_width > 0) {
                ...
                sidetext->SetFont(/*wxGetApp().normal_font()*/ANKER_FONT_NO_2);
    ...
    // std::cout << " ==--====activate_line , OG_ctrl size:" <<(custom_ctrl?custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl? custom_ctrl->GetSize().GetHeight():-1) << "    group:" << this->title << std::endl;
}

wxString AnkerOptionsGroup::get_OptionGroup_icon_name(const wxString& groupName)
{
    wxString str = groupName;
    str.Replace(" ", "_");
    return "OG_" + str;
}

// create all controls for the option group from the m_lines
bool AnkerOptionsGroup::activate(std::function<void()> throw_if_canceled/* = [](){}*/, int horiz_alignment/* = wxALIGN_LEFT*/)
{
    // std::cout << " ==activate 1 , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;
	if (sizer)//(!sizer->IsEmpty())
		return false;

	try {
        //std::cout << "AnkerOptionsGroup::activate,  staticbox:" << staticbox << "      group name:" << (title)<<"\t\t\t" << _(title) << "          m_parent:" << m_parent << std::endl;
        if (staticbox) {
            stb = new wxStaticBox(m_parent, wxID_ANY, _(title));
            if (!wxOSX) stb->SetBackgroundStyle(wxBG_STYLE_PAINT);
            stb->SetFont(wxOSX ? /*wxGetApp().normal_font()*/ANKER_FONT_NO_1 : /*wxGetApp().bold_font()*/ANKER_BOLD_FONT_NO_1);
            wxGetApp().UpdateDarkUI(stb);

            sizer = (staticbox ? new wxStaticBoxSizer((wxStaticBox*)stb, wxVERTICAL) : new wxBoxSizer(wxVERTICAL));
        }
        else {
            // add by dhf for Anker style OptionsGroup (AnkerOptionsGroup)
            stb = nullptr;
            sizer = (staticbox ? new wxStaticBoxSizer((wxStaticBox*)stb, wxVERTICAL) : new wxBoxSizer(wxVERTICAL));


            groupHeaderSizer = new wxBoxSizer(wxHORIZONTAL);
            {
                // group Icon
                if (!title.empty()) {
                    BlinkingBitmap* Bitmap = nullptr;
                    std::string OGIconName = get_OptionGroup_icon_name(title).ToStdString();
                    if (try_get_bmp_bundle(OGIconName)) {
                        Bitmap = new BlinkingBitmap(m_parent, OGIconName);
                    }
                    else {
                        std::cout << "  ---------------activate option group, no OG icon: " << OGIconName << "   Use the default Icon instead." << std::endl;
                        Bitmap = new BlinkingBitmap(m_parent);
                    }
                    Bitmap->activate();

                    if (!(this->title.empty()))
                        groupHeaderSizer->Add(Bitmap, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
                }

                groupHeaderSizer->AddSpacer(5);

                // group name
                wxStaticText* groupName = new wxStaticText(m_parent, wxID_ANY, _(title));
                groupName->SetForegroundColour(wxColour(255, 255, 255));
                groupName->SetBackgroundColour(wxColour(51, 52, 56));
                groupName->SetFont(ANKER_FONT_NO_1);
                groupHeaderSizer->Add(groupName, wxALIGN_CENTER_VERTICAL);

                groupHeaderSizer->AddSpacer(5);

                // Line
                wxPanel* splitLineCtrl = new wxPanel(m_parent, wxID_ANY);
                splitLineCtrl->SetBackgroundColour(wxColour(59, 61, 66));
                splitLineCtrl->SetMinSize(wxSize(1, 1));
                splitLineCtrl->SetMaxSize(wxSize(1, 1));

                groupHeaderSizer->Add(splitLineCtrl, 0, wxEXPAND | wxALIGN_CENTER_VERTICAL, 0);
            }
            sizer->Add(groupHeaderSizer);
        }
        ...
        //std::cout << "   AnkerOptionsGroup::activate,  "  << "      group name:" << _(title)  << std::endl;
		// activate lines
		for (AnkerLine& line: m_lines) {
			throw_if_canceled();
			activate_line(line);
		}
    ...
    // std::cout << " ==--====activate 2 , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;

	return true;
}
// delete all controls from the option group
void AnkerOptionsGroup::clear(bool destroy_custom_ctrl)
{
	if (!sizer)
		return;
...
AnkerLine AnkerOptionsGroup::create_single_option_line(const AnkerOption& option, const std::string& path/* = std::string()*/) const
{
    AnkerLine retval{ _(option.opt.label), from_u8(option.opt.tooltip) };
...
void AnkerOptionsGroup::clear_fields_except_of(const std::vector<std::string> left_fields)
{
...
void AnkerOptionsGroup::on_change_OG(const t_config_option_key& opt_id, const boost::any& value) {
	if (m_on_change != nullptr)
		m_on_change(opt_id, value);
}

AnkerOption AnkerConfigOptionsGroup::get_option(const std::string& opt_key, int opt_index /*= -1*/)
{
    // std::cout << " ==--====get_option , OG_ctrl size:" << (custom_ctrl ? custom_ctrl->GetSize().GetWidth() : -1) << "," << (custom_ctrl ? custom_ctrl->GetSize().GetHeight() : -1) << "    group:" << this->title << std::endl;

	if (!m_config->has(opt_key)) {
        ANKER_LOG_ERROR << "No " << opt_key << " in AnkerConfigOptionsGroup config.";
	}
...
void AnkerConfigOptionsGroup::on_change_OG(const t_config_option_key& opt_id, const boost::any& value)
{
	if (!m_opt_map.empty())
	{
		...
		if (it == m_opt_map.end())
		{
            AnkerOptionsGroup::on_change_OG(opt_id, value);
			return;
		}
...
void AnkerConfigOptionsGroup::back_to_initial_value(const std::string& opt_key)
{
...
void AnkerConfigOptionsGroup::back_to_sys_value(const std::string& opt_key)
{
...
void AnkerConfigOptionsGroup::back_to_config_value(const DynamicPrintConfig& config, const std::string& opt_key)
{
	boost::any value;
	...
	if (opt_key == "extruders_count") {
        ...
        AnkerOptionsGroup::on_change_OG(opt_key, value);
        return;
    }
	else
	{
...
void AnkerConfigOptionsGroup::on_kill_focus(const std::string& opt_key)
{
...
void AnkerConfigOptionsGroup::reload_config()
{
...
void AnkerConfigOptionsGroup::Hide()
{
    Show(false);
}

void AnkerConfigOptionsGroup::Show(const bool show)
{
...
bool AnkerConfigOptionsGroup::is_visible(ConfigOptionMode mode)
{
...
bool AnkerConfigOptionsGroup::update_visibility(ConfigOptionMode mode)
{
	if (m_options_mode.empty() || !m_grid_sizer)
		return true;

    if (custom_ctrl) {
        custom_ctrl->SetSize(wxSize(custom_ctrl->GetParent()->GetClientSize().GetWidth() - 10, -1));
        custom_ctrl->SetMaxSize(wxSize(custom_ctrl->GetParent()->GetClientSize().GetWidth() - 10, -1));
...
void AnkerConfigOptionsGroup::msw_rescale()
{
    ...
    for (const AnkerLine& line : m_lines) {
...
void AnkerConfigOptionsGroup::sys_color_changed()
{
#ifdef _WIN32
    if (staticbox && stb) {
...
void AnkerConfigOptionsGroup::refresh()
{
    if (custom_ctrl)
        custom_ctrl->Refresh();
}

boost::any AnkerConfigOptionsGroup::config_value(const std::string& opt_key, int opt_index, bool deserialize) {
...
boost::any AnkerConfigOptionsGroup::get_config_value(const DynamicPrintConfig& config, const std::string& opt_key, int opt_index /*= -1*/)
{
...
Field* AnkerConfigOptionsGroup::get_fieldc(const t_config_option_key& opt_key, int opt_index)
{
	Field* field = get_field(opt_key);
	if (field != nullptr)
		return field;
...
std::pair<Anker_OG_CustomCtrl*, bool*> AnkerConfigOptionsGroup::get_custom_ctrl_with_blinking_ptr(const t_config_option_key& opt_key, int opt_index/* = -1*/)
{
	...
	for (AnkerLine& line : m_lines)
		for (const AnkerOption& opt : line.get_options())
...
void AnkerConfigOptionsGroup::change_opt_value(const t_config_option_key& opt_key, const boost::any& value, int opt_index /*= 0*/)
...
wxString AnkerOptionsGroup::get_url(const std::string& path_end)
{
    if (path_end.empty())
...
bool AnkerOptionsGroup::launch_browser(const std::string& path_end)
{
    return wxGetApp().open_browser_with_warning_dialog(AnkerOptionsGroup::get_url(path_end), wxGetApp().mainframe->m_tabpanel);
}
...
bool AnkerOptionsGroup::is_option_without_field(const std::string& opt_key)
{
...
AnkerogStaticText::AnkerogStaticText(wxWindow* parent, const wxString& text) :
...
void AnkerogStaticText::SetText(const wxString& value, bool wrap/* = true*/)
{
	SetLabel(value);
...
void AnkerogStaticText::SetPathEnd(const std::string& link)
{
#ifndef __linux__

    Bind(wxEVT_ENTER_WINDOW, [this, link](wxMouseEvent& event) {
        // SetToolTip(AnkerOptionsGroup::get_url(get_app_config()->get("suppress_hyperlinks") != "1" ? link : std::string()));
        bool suppress_hyperlinks = true;
        SetToolTip(AnkerOptionsGroup::get_url(suppress_hyperlinks != true ? link : std::string()));
        FocusText(true);
        event.Skip();
    });
    ...
    Bind(wxEVT_LEFT_UP, [link, this](wxMouseEvent& event) {
        if (!HasCapture())
            return;
        ReleaseMouse();
        AnkerOptionsGroup::launch_browser(link);
        event.Skip();
    });
    ...
    Bind(wxEVT_MOTION, [link, this](wxMouseEvent& event) {
        SetToolTip(AnkerOptionsGroup::get_url(!get_app_config()->get_bool("suppress_hyperlinks") ? link : std::string()));
        FocusText(true);
        event.Skip();
    });
...
void AnkerogStaticText::FocusText(bool focus)
{
