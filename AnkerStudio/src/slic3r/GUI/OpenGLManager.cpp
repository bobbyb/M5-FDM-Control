    ...
    if (!m_gl_initialized) {
...
#if ENABLE_OPENGL_ES
                _L("M5 FDM Control requires OpenGL ES 2.0 capable graphics driver to run correctly, \n"
...
#elif ENABLE_GL_CORE_PROFILE
                _L("M5 FDM Control requires OpenGL %s capable graphics driver to run correctly, \n"
...
#else
                _L("M5 FDM Control requires OpenGL 2.0 capable graphics driver to run correctly, \n"
                    "while OpenGL version %s, render %s, vendor %s was detected."), s_gl_info.get_version_string(), s_gl_info.get_renderer(), s_gl_info.get_vendor());
#endif // ENABLE_OPENGL_ES
            message += "\n";
...
#ifdef _WIN32
            message += "\n";
            message += _L("As a workaround, you may run M5 FDM Control with a software rendered 3D graphics by running anker-studio.exe with the --sw-renderer parameter.");
#endif
        	wxMessageBox(message, wxString("M5 FDM Control - ") + _L("Unsupported OpenGL version"), wxOK | wxICON_ERROR);
        }
        ...
        if (valid_version) {
            ...
            if (!result) {
                ...
                wxMessageBox(message, wxString("M5 FDM Control - ") + _L("Error loading shaders"), wxOK | wxICON_ERROR);
            }
#if ENABLE_OPENGL_DEBUG_OPTION
