...
*/
    ...
    if (start_gui) {
        ...
        params.start_as_gcodeviewer = false; //  Troubleshoot the problem of restarting a process by right-clicking GCode in Windows.
...
std::string string_To_UTF8(const std::string& str)
{
    ...
    auto dataStyle = use_facet<boost::locale::info>(loc).encoding();
    std::string readlData = boost::locale::conv::between((char*)str.c_str(), (char*)str.c_str() + str.size(), dataStyle, "UTF8");
    return readlData;
}

std::string format_message(
    boost::log::value_ref< std::string, boost::log::expressions::tag::smessage > 
    const& message)
{    
    ...
    if (message)
    {
        ...
        if (bFlag)
        {
            std::string plaintValue(msg);
            int strLength = 0;
            plaintValue += '\n';
  
            plaintValue = string_To_UTF8(plaintValue);
            std::string ciphertext = std::string();

#if defined(_DEBUG) || !defined(NDEBUG)
            ciphertext = plaintValue;
#else
            logCryptText(plaintValue, strLength, ciphertext);
#endif
...
// boost log initialization output to file
void boostLogInit() {
    // Setting the log output file    
    std::string pidStr;
#ifdef WIN32
    DWORD processId = GetCurrentProcessId();
    pidStr = std::to_string(processId);
#else
    ...
    auto fileSink = logging::add_file_log(
        ...
        keywords::format = boost::log::expressions::stream
        << boost::phoenix::bind(&format_message, boost::log::expressions::smessage.or_none()),
        keywords::min_free_space = 1024 * 1024 * 1024 // set min free space to 1GB
    );

    std::locale loc = boost::locale::generator()("en_US.UTF-8");
    fileSink->imbue(loc);

    // Add some default properties, otherwise the things set above may not take effect, such as timestamps
    logging::add_common_attributes();
    
#if defined(_DEBUG) || !defined(NDEBUG)
    std::cout << "debug not create the key";
#else   
    uint8_t* data = nullptr;
    uint32_t len = 0;        

    logCreateKeyBlock(&data, len);

    char tmpData[ENC_TEXT_DATA_SIZE_MAX] = { 0 };
    ...
    if (data)
    {
        ...
        for (int i = 0; i < len; i++)
            logContent += tmpData[i];
    }

    if(data)
    {
        std::vector<uint8_t> strVec;
        strVec.assign(data, data + len);        
        BOOST_LOG_TRIVIAL(info) << logContent;
    }
#endif    
}
...
bool CLI::setup(int argc, char **argv)
{
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into AnkerStudio process (for example Nahimic, see GH #5573).
    ...
    ANKER_LOG_INFO << "set all resouce dir";
...
extern "C" {
    // Enable printing stacktrace by default. It can be disabled by running AnkerStudio with "UBSAN_OPTIONS=print_stacktrace=0".
...
extern "C" {
    ...
    __declspec(dllexport) LONG __stdcall AnkerExceptionHandler(EXCEPTION_POINTERS* pException)
    {
       ...
       // sw.Walk();
        ...
        //carsh info upload
        Slic3r::GUI::GUI_App* gui_app = dynamic_cast<Slic3r::GUI::GUI_App*>(Slic3r::GUI::GUI_App::GetInstance());
        if (gui_app) {
            gui_app->on_CrashTrack();
        }
...
{
    //do nothing,to ignore this sign
}
...
int main(int argc, char **argv)
{
    ...
    buryMap.insert(std::make_pair(c_ss_status, "start"));
